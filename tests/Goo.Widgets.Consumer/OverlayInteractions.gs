...
func OverlayContracts() {
  ...
  try {
    var rejected bool
    try { probe.Render(PopoverInput{Open: true}) } catch (error ArgumentException) { rejected = true }
    Require(rejected, "Popover accepted an open popup without an anchor or point")
    rejected = false
    try { probe.Render(PopoverInput{Open: true, WindowPoint: Point{}, Width: Double.NaN}) }
    catch (error ArgumentException) { rejected = true }
    Require(rejected, "Popover accepted a nonfinite size")
    var activations int32
    let visual = (ModalDialog{Open: true, OnConfirm: () -> { activations++ }, ConfirmDisabled: true,
      CreateConfirm: (input ModalDialog, content Blob) -> Button{Children: {content}}}.Build() as Container)!!
  ...
  let menu = MenuProbe()
  var rejected bool
  try { menu.Render(MenuInput{Items: []MenuItem{MenuItem{Id: "same"}, MenuItem{Id: "parent", Children: []MenuItem{MenuItem{Id: "same"}}}}}) }
  catch (error ArgumentException) { rejected = true }
...
internal class OverlayHost : Cell {
  ...
  private func CloseOuter() { DialogOpen = false
    Rebuild() }
  private func CloseTop() { TopOpen = false
    Rebuild() }
  private func Confirm() { Confirms++
    DialogOpen = false
    Rebuild() }
  private func PopupDismiss(reason PopoverDismissReason) { LastDismiss = reason
    Dismisses++
    PopupOpen = false
    Rebuild() }
  private func MenuDismiss() { MenuOpen = false
    Rebuild() }
  private func MenuActivate(id string) { Activated = id
    Rebuild() }
  ...
  private func Cancel(input ModalDialog, content Blob) Button {
    let result = Button{Handle: ElementHandle(), Focusable: !EmptyDialog, Padding: 10.0,
      BackgroundColor: "#27272a", BorderRadius: 5.0, Children: {content}}
    CancelButton = result
    return result
  }
  private func ConfirmFactory(input ModalDialog, content Blob) Button {
    let result = Button{Handle: ElementHandle(), Focusable: !EmptyDialog, Padding: 10.0,
      BackgroundColor: "#6366f1", BorderRadius: 5.0, Children: {content}}
    ConfirmButton = result
    return result
  }
  private func CapturePanel(input PopoverInput, bounds ElementRect, content Blob) Container {
    let result = Container{Padding: 14.0, BackgroundColor: "#1e2130", BorderWidth: 1.0, BorderColor: "#6366f1", BorderRadius: 9.0}
    PopupPanel = result
    return result
  }
  ...
  private func MenuRow(input MenuInput, item MenuItem, prepared Button) Button { MenuRows[item.Id!!] = prepared
    return prepared }

  public override func Build() Blob {
    let root = Container{Width: Length.Percent(100), Height: Length.Percent(100), BackgroundColor: "#111318", Padding: 24.0, Gap: 16.0,
      Color: "#fafafa", OnKeyDown: (event KeyEvent) -> { BackgroundKeys++ }, Children: {
        Text{Key: "title", Content: "Managed overlays", FontSize: 26.0, FontWeight: 700},
        Text{Key: "description", Content: "Nested dialogs · anchored popovers · accessible context menus", FontSize: 14.0, Color: "#a1a1aa"},
        Button{Key: "before", Handle: Before, Width: 180.0, Height: 38.0, BackgroundColor: "#27272a", BorderRadius: 6.0,
          OnClick: () -> { BackgroundClicks++ }, Accessibility: Accessibility{Name: "Background action"}, Children: {Text{Content: "Background action"}}},
      }}
    if ShowAnchor { root.Children.Add(Button{Key: "anchor", Handle: Anchor, Position: PositionType.Absolute,
      Left: AnchorX, Top: AnchorY, Width: 180.0, Height: 38.0, BackgroundColor: "#4f46e5", BorderRadius: 6.0,
      OnClick: () -> { PopupOpen = true
        Rebuild() }, Accessibility: Accessibility{Name: "Open popup"}, Children: {Text{Content: "Open popup"}}}) }
    if ShowDialogs {
      let content = if EmptyDialog { Blob(Text{Content: "No focusable children. Escape still closes this dialog."}) }
      else { Container{Gap: 12.0, Children: {
        Text{Content: "Focus stays in this dialog until it closes.", Color: "#a1a1aa"},
        TextEntry{Handle: Edit, Value: "Release notes", Width: Length.Percent(100), Height: 38.0,
          Padding: 8.0, BackgroundColor: "#09090b", Accessibility: Accessibility{Name: "Dialog editor"}},
        Button{Handle: Nested, Height: 36.0, BackgroundColor: "#312e81", OnClick: () -> { TopOpen = true
          Rebuild() }, Accessibility: Accessibility{Name: "Open nested dialog"}, Children: {Text{Content: "Open nested dialog"}}},
      }} }
      root.Children.Add(Cell.Mount[ModalDialog, ModalDialogHost]("outer", ModalDialog{Open: DialogOpen,
        Header: Text{Content: "Review change", FontSize: 21.0, FontWeight: 700}, Content: content,
        AccessibilityName: "Review change", Width: 440.0, OnClose: CloseOuter, OnCancel: CloseOuter, OnConfirm: Confirm,
        CreateRoot: Outer, CreateCancel: Cancel, CreateConfirm: ConfirmFactory}))
      root.Children.Add(Cell.Mount[ModalDialog, ModalDialogHost]("top", ModalDialog{Open: TopOpen,
        Header: Text{Content: "Nested confirmation", FontSize: 21.0, FontWeight: 700}, Width: 370.0,
        Content: Text{Content: "Only this top dialog receives keyboard input.", Color: "#a1a1aa"},
        AccessibilityName: "Nested confirmation", OnClose: CloseTop, OnCancel: CloseTop, OnConfirm: CloseTop, CreateRoot: Top}))
    }
    var popup = PopoverInput{Open: PopupOpen, AccessibilityName: "Quick details", Width: 280.0, MaxHeight: 220.0,
      Content: Container{Gap: 14.0, Children: {
        ...
        Button{Handle: PopupAction, Height: 34.0, BackgroundColor: "#4f46e5", Children: {Text{Content: "Continue"}}, Accessibility: Accessibility{Name: "Popup action"}},
      }}, OnDismiss: PopupDismiss, CreatePanel: CapturePanel, CreateRoot: CapturePopup}
    if PointPopup { popup.WindowPoint = Point{X: 800.0, Y: 520.0} } else { popup.Anchor = Anchor }
    root.Children.Add(Cell.Mount[PopoverInput, Popover]("popup", popup))
    root.Children.Add(Cell.Mount[MenuInput, Menu]("menu", MenuInput{Open: MenuOpen,
      WindowPoint: Point{X: 630.0, Y: 180.0}, Width: 220.0, AccessibilityName: "Actions",
      Items: []MenuItem{
        MenuItem{Id: "open", Label: "Open"}, MenuItem{Id: "disabled", Label: "Unavailable action", Disabled: true},
        MenuItem{Id: "separator", Separator: true},
        MenuItem{Id: "tools", Label: "Tools", Children: []MenuItem{
          MenuItem{Id: "copy", Label: "Copy address"}, MenuItem{Id: "offline", Label: "Offline action", Disabled: true},
          MenuItem{Id: "advanced", Label: "Advanced", Children: []MenuItem{MenuItem{Id: "inspect", Label: "Inspect details"}}},
        }}, MenuItem{Id: "delete", Label: "Delete"},
      }, OnActivate: MenuActivate, OnDismiss: MenuDismiss, DismissOnActivate: !KeepMenu, CreateItem: MenuRow}))
    return root
  }
}

func OverlayFocus(adapter SearchListSemantics, name string) bool -> FindSemantics(adapter.Tree?.Root, name)?.Focused ?? false

func OverlayInteractions() {
  OverlayContracts()
  let host = OverlayHost()
  let semantics = SearchListSemantics()
  let window = Window{Title: "Overlay verification", Width: 860, Height: 560, Root: host, AccessibilityAdapter: semantics}
  window.Open()
  try {
    ...
    Require(!host.Before.Focus() && !window.PerformAccessibilityAction(oldBackground, AccessibilityActionRequest(AccessibilityAction.Activate)), "Modal permitted background focus or accessibility activation")
    Require(FindSemantics(semantics.Tree?.Root, "Background action") == nil, "Modal exposed background accessibility content")
    ...
    Require(!host.TopOpen && host.DialogOpen && OverlayFocus(semantics, "Open nested dialog"), "Closing nested dialog failed to restore its trigger")
    MouseButton(id, 12.0F, 12.0F, true)
    MouseButton(id, 12.0F, 12.0F, false)
    PumpFrames(window, 10)
    Require(!host.DialogOpen && OverlayFocus(semantics, "Background action"), "Backdrop dismissal failed to restore prior focus")
    ...
    Require(panel.Y + panel.Height <= anchor.Y + .5 && panel.X >= 8.0 && panel.X + panel.Width <= 852.5,
    ...
    SendKey(id, SDLScancode.Escape)
    PumpFrames(window, 8)
    Require(!host.PopupOpen && host.LastDismiss == PopoverDismissReason.Escape && OverlayFocus(semantics, "Open popup"), "Popover Escape/restore failed")
    ...
    Require(narrow.X >= 7.5 && narrow.X + narrow.Width <= 432.5 && narrow.Y + narrow.Height <= 552.5, "Point popup escaped resized bounds")
    CaptureIssueProof(window, "popover-point-narrow")
    MouseButton(id, 12.0F, 12.0F, true)
    MouseButton(id, 12.0F, 12.0F, false)
    PumpFrames(window, 8)
    Require(!host.PopupOpen && host.LastDismiss == PopoverDismissReason.OutsideClick && host.BackgroundClicks == 0, "Outside click failed to dismiss without activating background")
    window.Width = 860
    ...
    host.Rebuild()
    PumpFrames(window, 14)
    Require(!host.PopupOpen && host.LastDismiss == PopoverDismissReason.AnchorRemoved, "Removed anchor retained its popup lifecycle")
    host.Before.Focus()
    host.MenuOpen = true
    host.Rebuild()
    PumpFrames(window, 16)
    ...
    PumpFrames(window, 14)
    Require(FindSemantics(semantics.Tree?.Root, "Copy address") == nil, "Submenu blocked pointer movement back to its parent")
    ...
    Require(!host.MenuOpen && OverlayFocus(semantics, "Background action"), "Menu activation failed to dismiss/restore")
    Require(host.BackgroundClicks == 0 && host.Confirms == 0, "An overlay accidentally activated a background or confirmation action")
  } finally { window.RequestClose()
    PumpFrames(window, 5) }
  Console.WriteLine("PASS: native managed modal entry/containment/nesting/fallback/backdrop/unmount, popover anchor/point/flip/resize/dismiss/restore, menu disabled/separator/keyboard/submenu/hover/activation policy")
}
