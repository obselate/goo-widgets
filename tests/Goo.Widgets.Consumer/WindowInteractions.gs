    ...
    public override func Build() Blob -> Container(){
        ...
        Cell.Mount[GraphCanvasInput, GraphCanvas](
            ...
            GraphCanvasInput{
                ...
                Width: 500.0,
                Height: 320.0,
                CustomizeNode: (node GraphNode, card Container) -> {
                    card.Handle = ElementHandle{}
                    Card = card
                    return card
                },
                ...
                OnNodePositionChanged: (change GraphNodePositionChange) -> {
        ...
        let root = Container(){
            ...
            Cell.Mount[SliderInput, Slider](
                ...
                SliderInput{
                    ...
                    CreateRoot: (input SliderInput, slider Container) -> {
                        SliderRoot = slider
                        return slider
                    },
                }
...
func SendKey(windowId uint32, key SDLScancode) {
    var down = SDLEvent{
        Key: SDLKeyboardEvent{Type: SDLEventType.KeyDown, WindowID: windowId, Scancode: key, Down: uint8(1),}
    }
    var up = SDLEvent{Key: SDLKeyboardEvent{Type: SDLEventType.KeyUp, WindowID: windowId, Scancode: key,}}
...
func WindowInteractions() {
    ...
    try {
        ...
        PumpFrames(window, 3)
        Require(
            host.Value == 1.0 && host.Commits == 2,
            "Captured pointer drag did not clamp and commit at the endpoint. Value=" + host.Value.ToString() +
                ", commits=" +
                host
                .Commits
                .ToString() + ", width=" + box.Width.ToString()
        )
        ...
        Require(
            host.SliderRoot!!.Handle!!.BorderBox.Width > box.Width,
            "Slider layout did not respond to window resize."
        )
        ...
        PumpFrames(window, 3)
        Require(
            graph.Selected.Length == 1 && graph.Selected[0] == "one",
            "Native graph selection lost its node identity."
        )
        Require(
            graph.Moves > 0 && graph.Position.X > 190.0 && graph.Position.Y > 150.0,
            "Captured native graph drag did not update host coordinates."
        )
        Console.WriteLine(
            "PASS: actual window, native keyboard, captured pointer, disabled input, color change, resize, disposal/remount, graph selection/drag."
        )
    } finally {
        window.RequestClose()
        PumpFrames(window, 3)
