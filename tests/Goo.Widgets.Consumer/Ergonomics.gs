...
func ColorPickerErgonomics() {
  ...
  Require(full.Children.Count == 4,
    "ColorPicker did not build its full default composition.")
  let defaultModes = (full.Children[0] as Container)!!
  Require(defaultModes.Children.Count == 3 && defaultModes.Children[2].Accessibility?.Selected == true,
    "ColorPicker did not default to the OKLCH model.")
  fullPicker.Dispose()

  let compactPicker = ColorPickerProbe{}
  let compact = (compactPicker.Render(Goo.Widgets.Colors.ColorPickerInput{Value: 0x336699, Compact: true}) as Container)!!
  Require(compact.Children.Count == 2,
    "Compact ColorPicker did not omit its selector and preview.")
  compactPicker.Dispose()

  let disabledPicker = ColorPickerProbe{}
  var disabledChanges int32 = 0
  let disabled = (disabledPicker.Render(Goo.Widgets.Colors.ColorPickerInput{
    Value: 0x336699,
    Mode: ColorMode.Hsv,
    Disabled: true,
    OnModeChanged: (mode ColorMode) -> { disabledChanges++ },
  }) as Container)!!
  let disabledModes = (disabled.Children[0] as Container)!!
  let disabledHsl = (disabledModes.Children[0] as Button)!!
  Require(disabledHsl.Disabled && disabledHsl.Focusable == false,
    "Disabled ColorPicker left a mode control enabled.")
  disabledHsl.OnClick?.Invoke()
  Require(disabledChanges == 0 && disabledModes.Children[1].Accessibility?.Selected == true,
  ...
  let customInput = Goo.Widgets.Colors.ColorPickerInput{
    Value: 0x808080,
    Mode: ColorMode.Hsv,
    OnValueChanged: (rgb int32) -> { changedRgb = rgb },
    CreateRoot: (resolved Goo.Widgets.Colors.ColorPickerInput, wheel Blob, slider Blob) -> {
      ...
      customWheel = wheel
      return Container{Children: {wheel, slider}}
    },
  }
  ...
  customPicker.Render(customInput)
  Require(rootRgb == changedRgb && rootMode == ColorMode.Hsv,
...
func CheckboxErgonomics() {
  ...
  let input = Checkbox{
    ...
    Label: "Enable alerts",
    OnChange: (state AccessibilityChecked) -> { next = state },
  }
  let before = (input.Build() as Button)!!
  Require(before.Children.Count == 2 && (before.Children[1] as Text)!!.Content == "Enable alerts",
  ...
  Require(after.Accessibility?.Checked == AccessibilityChecked.True,
...
func SliderErgonomics() {
  ...
  Require(beforeHeader.Children.Count == 2 && (beforeHeader.Children[0] as Text)!!.Content == "Volume"
  ...
  Require((afterHeader.Children[1] as Text)!!.Content == "100%",
    "Slider did not update its visible formatted value.")
}
...
func GraphNodeCardErgonomics() {
  ...
  Require(fromNode.Key == "graph-node-node",
    "GraphNodeCard did not build with Node.Position as its default center.")
  ...
  } catch (error ArgumentOutOfRangeException) { defaultRejected = true }
  ...
  Require(explicit.Key == "graph-node-node",
