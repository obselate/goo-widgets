...
internal class SearchListSemantics : AccessibilityAdapter {
  ...
  public func Update(tree AccessibilityTree) { Tree = tree }
}
...
internal class SearchListHost : Cell {
  ...
  private func Select(id string) { Selected = id
    Activations++
    Rebuild() }
  private func SelectAlternate(id string) { Selected = id
  ...
  public override func Build() Blob -> Container {
    Padding: 24, Gap: 16, BackgroundColor: "#09090b", Color: "#fafafa",
    Children: {
      Text{Content: "SearchList · controlled selection", FontSize: 22},
      SearchList{
        Items: items, SelectedId: Selected, Width: 400, Height: 240, RowHeight: Height,
        SelectedColor: Selection, TextColor: Foreground,
        OnSelect: if Alternate { selectAlternate } else { selectionCallback },
        CustomizeRow: if Custom { customRow } else { row },
      }.Build(),
...
func FindSemantics(node AccessibilityNode?, name string) AccessibilityNode? {
  ...
  for child in node.Children {
    if let found = FindSemantics(child, name) { return found }
  }
  return nil
}
...
func SearchListInteractions() {
  ...
  try {
    ...
    Require(window.PerformAccessibilityAction(beta.Id, AccessibilityActionRequest(AccessibilityAction.Activate)),
    ...
    Require(FindSemantics(semantics.Tree?.Root, "Alpha")!!.Selected == false
    ...
    Require(FindSemantics(semantics.Tree?.Root, "Alpha")!!.Id == alpha.Id
    ...
    host.Rebuild()
    PumpFrames(window, 4)
    Require(host.Rows["beta"].Children.Count == 3,
      "Retained rows ignored changed colors or row customization.")
    Require(Math.Abs(host.Handles["beta"].BorderBox.Height - 52.0) < 1.0,
      "Retained row ignored the new row height.")
    ClickNativeButton(window, OnlyNativeWindow(), host.Rows["alpha"])
    Require(host.Selected == "alpha" && host.AlternateActivations == 1 && host.Activations == 1,
    ...
    Console.WriteLine("PASS: SearchList retained selection, stable identity, unchanged row reuse, updated colors/height/callback/customization, disabled input.")
  } finally {
    window.RequestClose()
    PumpFrames(window, 3)
  }
