...
func WindowChromeContracts() {
    let row = (
        WindowChrome{CreateRoot: ChromeContractRoot, CreateControl: ChromeContractControl}.Build() as Container
    )!!
    Require(
        !row.Focusable && row.Children.Count == 4,
        "Chrome changed the factory child order or became a focusable drag region"
    )
    ...
    for child in row.Children {
        if child is Button {
            ...
            Require(
                child.Children.Count == 1 && child.Children[0] is Shape,
                "A custom chrome control bypassed the content factory"
            )
        }
    }
}
...
internal class ChromeHost : Cell {
    ...
    private func Menu(input WindowChrome, prepared MenuInput) MenuInput {
        MenuFactories++
        MenuInput = prepared
            ...
            var chromeInput = WindowChrome{
                ...
                LeadingContent: Container(){
                    ...
                    Button(){
                        ...
                        Text{Content: "Embedded control", FontSize: 12.0}
                    }
                },
            }
            if NativeBinding {
                chromeInput.Host = window
            } else {
...
func ChromeSeat(command string) {
    Require(
        Environment.GetEnvironmentVariable("WAYLAND_DISPLAY") == "goo-widgets",
        "Chrome native input requires the isolated compositor"
    )
...
func ChromeInteractions() {
    ...
    let window = Window{
        Title: "Window chrome verification",
        Width: 1400,
        Height: 900,
        Decorated: false,
        Root: host,
    ...
    try {
        ...
        Require(
            host.Blank.IsMounted && host.Embedded.IsMounted && host.Controls.Count == 3,
            "Custom chrome root lost its required children"
        )
        ...
        Require(
            window.State == WindowState.Maximized && host.Commands == 1 && host.LastCommand == "Maximize callback",
            "Native titlebar double-click did not use the maximize override"
        )
        CaptureIssueProof(window, "chrome-maximized")
        ChromeNativeDoubleClick(window)
        Require(
            window.State == WindowState.Normal && host.Commands == 2 && host.LastCommand == "Restore callback",
            "Native titlebar double-click did not restore through the callback"
        )
        CaptureIssueProof(window, "chrome-normal")
        CompositeClick(window, host.Embedded)
        CompositeClick(window, host.Embedded)
        Require(
            host.EmbeddedClicks == 2 && host.Commands == 2 && window.State == WindowState.Normal,
            "Embedded titlebar clicks triggered the blank gesture"
        )
        CompositeClick(window, host.Controls[WindowChromeAction.Maximize].Handle!!)
        PumpFrames(window, 25)
        Require(
            window.State == WindowState.Maximized && host.Commands == 3 && host.BadClicks == 0,
            "Custom maximize control bypassed the resolved callback"
        )
        ...
        Require(
            !host.MenuInput.Open && OverlayFocus(semantics, "Embedded control"),
            "Window menu dismissal did not restore focus"
        )
        ChromeSeat("move 600 20")
        PumpFrames(window, 3)
        ...
        PumpFrames(window, 20)
        Require(
            host.MenuInput.Open && host.MenuInput.WindowPoint != nil,
            "Native right-click did not open a pointer-positioned command menu"
        )
        ...
        Require(
            !host.MenuInput.Open && host.LastCommand == "Close callback" && host.Commands == 5 && window.IsOpen,
        ...
        ChromeNativeDoubleClick(window)
        Require(
            window.State == WindowState.Maximized && host.Commands == 5,
            "Native titlebar host default did not maximize"
        )
        Require(host.Embedded.Focus(), "Maximized chrome control could not focus")
        SendKey(id, SDLScancode.Application)
        PumpFrames(window, 20)
        Require(
            host.MenuInput.Open && !host.MenuInput.Items!![0].Disabled && host.MenuInput.Items!![2].Disabled,
        ...
        ChromeNativeDoubleClick(window)
        Require(
            window.State == WindowState.Normal && host.Commands == 5,
            "Native titlebar host default did not restore"
        )
    ...
    Console.WriteLine(
        "PASS: native titlebar double-click/default/override, custom controls, embedded input, native pointer and keyboard menus, state transitions, focus restoration and removal"
    )
}
...
func ChromeCallbackInteractions() {
    ...
    try {
        ...
        PumpFrames(window, 20)
        Require(
            host.Commands == 1 && window.State == WindowState.Maximized,
            "Callback-only chrome did not use the shared pointer click count"
        )
        Require(host.Embedded.Focus(), "Callback-only embedded control could not focus")
        SendKey(id, SDLScancode.Application)
        PumpFrames(window, 15)
        Require(
            host.MenuInput.Open && host.MenuInput.Viewport?.Width > 0.0,
            "Callback-only chrome menu did not use measured overlay bounds"
        )
