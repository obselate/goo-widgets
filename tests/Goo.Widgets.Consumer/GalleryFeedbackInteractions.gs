...
func FeedbackNodes(root AccessibilityNode?, role AccessibilityRole, name string, found List[AccessibilityNode]) {
  ...
  if root.Role == role && (name == "" || root.Name == name || (role == AccessibilityRole.Text && root.Value == name)) { found.Add(root) }
  for child in root.Children { FeedbackNodes(child, role, name, found) }
}
...
func FeedbackSetValue(window Window, node AccessibilityNode, value string) {
  Require(window.PerformAccessibilityAction(node.Id, AccessibilityActionRequest.SetValue(value)),
...
func FeedbackCombo(window Window, adapter SearchListSemantics) {
  ...
  Console.WriteLine("combo-height before=" + before.ToString() + " filtered=" + filtered.ToString() + " cleared=" + restored.ToString())
  Require(Math.Abs(restored - before) < .1 && filtered < before, "Gallery ComboBox did not restore its popup height after clearing search")
  FeedbackSetValue(window, FeedbackNode(adapter, AccessibilityRole.SearchBox, "Search options"), "no such region")
  FeedbackSetValue(window, FeedbackNode(adapter, AccessibilityRole.SearchBox, "Search options"), "")
  Require(Math.Abs(FeedbackNode(adapter, AccessibilityRole.List, "Region").Bounds.Height - before) < .1,
...
func FeedbackSplit(window Window, adapter SearchListSemantics) {
  for index in 0 ... 2 {
    ...
    Console.WriteLine("split index=" + index.ToString() + " before=" + before.ToString() + " after=" + after.ToString())
...
func FeedbackChrome(window Window, adapter SearchListSemantics) {
  ...
  Require(FeedbackNode(adapter, AccessibilityRole.Text, "Maximized").Value == "Maximized", "Gallery title text did not bubble double-click into WindowChrome")
  FeedbackClick(window, FeedbackNode(adapter, AccessibilityRole.Button, "Embedded control").Bounds, 2)
  Require(FeedbackNode(adapter, AccessibilityRole.Text, "Maximized").Value == "Maximized", "Embedded chrome control also toggled window state")
}
...
func FeedbackDisclosure(window Window, adapter SearchListSemantics) {
  ...
  Require(window.PerformAccessibilityAction(header.Id, AccessibilityActionRequest(AccessibilityAction.Focus)), "Custom disclosure could not focus")
  PumpFrames(window, 10)
...
func FeedbackMedia(window Window, adapter SearchListSemantics) {
  ...
  Console.WriteLine("volume label-right=" + (label.X + label.Width).ToString() + " thumb-left=" + (track.X - 10.0).ToString())
...
func FeedbackWheel(window Window, bounds ElementRect, direction float32) {
  var event = SDLEvent{Wheel: SDLMouseWheelEvent{
    Type: SDLEventType.MouseWheel, WindowID: OnlyNativeWindow(), Which: 1u,
    MouseX: float32(bounds.X + bounds.Width / 2.0), MouseY: float32(bounds.Y + bounds.Height - 30.0), Y: direction,
  }}
...
func FeedbackGrid(window Window, adapter SearchListSemantics) {
  ...
  Require(window.PerformAccessibilityAction(filter.Id, AccessibilityActionRequest(AccessibilityAction.Focus)), "Grid filter focus failed")
  ...
  Console.WriteLine("grid-scrolled fjord selected=" + clickedAfter.Selected.ToString() + " before-y=" + clicked.Y.ToString() + " after-y=" + clickedAfter.Bounds.Y.ToString())
  Require(clickedAfter.Selected == true && Math.Abs(clickedAfter.Bounds.Y - clicked.Y) < .1,
  ...
  Require(partial.Y < grid.Y + grid.Height && partial.Y + partial.Height > grid.Y + grid.Height,
  ...
  Require(partialAfter.Selected == true && Math.Abs(partialAfter.Bounds.Y - partial.Y) < .1,
  ...
  for name in []string {"borealis", "cirrus", "ember"} {
    ...
    Console.WriteLine("grid-check " + name + " selected=" + current.Selected.ToString() + " before-y=" + box.Y.ToString() + " after-y=" + current.Bounds.Y.ToString())
  ...
  for name in []string {"atlas", "borealis", "cirrus", "ember", "fjord", "grove"} {
    Require(FeedbackNode(adapter, AccessibilityRole.Row, name).Selected == true, "Sorting transferred selected identity: " + name)
  }
  FeedbackSetValue(window, filter, "borealis")
  Require(FeedbackNode(adapter, AccessibilityRole.Row, "borealis").Selected == true, "Filtering lost selected identity")
  ...
  Require(window.PerformAccessibilityAction(ember.Id, AccessibilityActionRequest(AccessibilityAction.Expand)), "Grid detail expansion failed")
  PumpFrames(window, 16)
  CaptureIssueProof(window, "gallery-grid-filter-clear-expanded")
  for name in []string {"atlas", "borealis", "cirrus", "ember", "fjord", "grove"} {
    Require(FeedbackNode(adapter, AccessibilityRole.Row, name).Selected == true, "Filtering or detail expansion transferred selected identity: " + name)
  }
}
...
func FeedbackTree(window Window, adapter SearchListSemantics) {
  for name in []string {"Documents", "All hosts"} {
    ...
    Require(collapsed.Expanded == false && collapsed.Checked == checked, "Whole-row activation did not collapse without changing checks: " + name)
    SendKey(OnlyNativeWindow(), SDLScancode.Return)
    PumpFrames(window, 12)
    Require(FeedbackNode(adapter, AccessibilityRole.TreeItem, name).Expanded == true, "Enter did not expand tree branch")
    Require(window.PerformAccessibilityAction(collapsed.Id, AccessibilityActionRequest(AccessibilityAction.Activate)), "Tree activation failed")
    PumpFrames(window, 12)
    Require(FeedbackNode(adapter, AccessibilityRole.TreeItem, name).Expanded == false, "Accessible activation did not collapse tree branch")
  }
  let hosts = FeedbackNode(adapter, AccessibilityRole.TreeItem, "All hosts")
  Require(window.PerformAccessibilityAction(hosts.Id, AccessibilityActionRequest(AccessibilityAction.Expand)), "Tree expand failed")
  PumpFrames(window, 12)
  ...
  Require(checked.Expanded == true && checked.Checked == AccessibilityChecked.True,
    "Checkbox click also collapsed the branch or failed the host check policy")
  SendKey(OnlyNativeWindow(), SDLScancode.Space)
  PumpFrames(window, 12)
  Require(checked.Expanded == true && checked.Checked == AccessibilityChecked.False,
...
func FeedbackMenu(window Window, adapter SearchListSemantics) {
  ...
  Require(window.PerformAccessibilityAction(tools.Id, AccessibilityActionRequest(AccessibilityAction.Focus)), "Menu branch focus failed")
...
func GalleryFeedbackInteractions() {
  ...
  var visited = false
  for page in []string {"Combo box", "Split pane", "Window chrome", "Disclosure", "Media transport", "Data grid", "Tree view", "Menu"} {
    if page != Environment.GetEnvironmentVariable("GOO_WIDGETS_GALLERY_FEEDBACK_PAGE") { continue }
    visited = true
    let registry = CreateRegistry()
    let adapter = SearchListSemantics()
    let window = Window{Title: "Gallery feedback · " + page, Width: 1100, Height: 800,
      Root: Gallery(registry, registry.IndexOf(page)), AccessibilityAdapter: adapter}
    window.Open()
    try {
      PumpFrames(window, 24)
      if page == "Combo box" { FeedbackCombo(window, adapter) }
      else if page == "Split pane" { FeedbackSplit(window, adapter) }
      else if page == "Window chrome" { FeedbackChrome(window, adapter) }
      else if page == "Disclosure" { FeedbackDisclosure(window, adapter) }
      else if page == "Media transport" { FeedbackMedia(window, adapter) }
      else if page == "Data grid" { FeedbackGrid(window, adapter) }
      else if page == "Tree view" { FeedbackTree(window, adapter) }
      else if page == "Menu" { FeedbackMenu(window, adapter) }
