...
func ControllerDisposed(controller TextEditorController) bool -> bool(typeof(TextEditorController).GetField("disposed", BindingFlags.Instance | BindingFlags.NonPublic)!!.GetValue(controller)!!)
...
func TabBarContracts() {
  ...
  let root = (tabs.Render(TabBarInput{
    SelectedId: "one", Items: []NavigationItem{NavigationItem{Id: "one", Label: "One"}, NavigationItem{Id: "two", Label: "Two"}},
    CreateItem: (input TabBarInput, item NavigationItem, prepared Button) -> Button{BorderRadius: 17, Children: {Text{Content: item.Label!!}}},
  }) as Container)!!
  let first = (root.Children[0] as Button)!!
  let second = (root.Children[1] as Button)!!
  Require(first.Key == "one" && first.TabStop && first.Focusable && first.Accessibility?.Selected == true && !second.TabStop,
    "Tab item customization replaced required selection/focus wiring")
  var rejected bool
  try { tabs.Render(TabBarInput{Items: []NavigationItem{NavigationItem{Id: "same"}, NavigationItem{Id: "same"}}}) }
...
internal class CompositeHost : Cell {
  ...
  private func Divide(value float64) { Split = value
    Rebuild() }
  private func TabSelected(id string) { Selected = id
    Rebuild() }
  private func TextChanged(value string) { Value = value
    Changes++
    Rebuild() }
  private func CaptureDivider(input SplitPaneInput, root Container) Container { Divider = root
    return root }
  private func CaptureSplit(input SplitPaneInput, root Container) Container { SplitRoot = root
    return root }
  private func CaptureTab(input TabBarInput, item NavigationItem, button Button) Button { Tabs[item.Id!!] = button
    return button }
  private func CaptureTabs(input TabBarInput, root Container) Container { root.Handle = TabViewport
    TabRoot = root
    return root }
  private func CaptureEditor(input TextAreaInput, editor TextEditor) TextEditor { editor.Handle = EditorViewport
    Editor = editor
    return editor }

  public override func Build() Blob {
    let root = Container{Padding: 24, Gap: 18, Width: Length.Percent(100), Height: Length.Percent(100), BackgroundColor: "#09090b", Color: "#fafafa", Children: {
      Text{Key: "heading", Content: "Composite widgets", FontSize: 24},
      Cell.Mount[TabBarInput, TabBar]("tabs", TabBarInput{
        SelectedId: Selected, OnSelect: TabSelected, Activation: Manual ? TabActivation.Manual : TabActivation.Automatic,
        CreateItem: CaptureTab, CreateRoot: CaptureTabs,
        Items: []NavigationItem{
          NavigationItem{Id: "one", Label: "Overview"}, NavigationItem{Id: "locked", Label: "Unavailable", Disabled: true},
          NavigationItem{Id: "three", Label: "Activity"}, NavigationItem{Id: "four", Label: "Documents and history"},
          NavigationItem{Id: "five", Label: "Preferences"}, NavigationItem{Id: "six", Label: "Connections"},
        },
      }),
      Cell.Mount[SplitPaneInput, SplitPane]("split", SplitPaneInput{
        Value: Split, OnChange: Divide, OnCommit: (value float64) -> { Commits++ },
        Orientation: Vertical ? SplitOrientation.Vertical : SplitOrientation.Horizontal,
        Height: 180, MinimumFirst: 90, MinimumSecond: 80, CreateHandle: CaptureDivider, CreateRoot: CaptureSplit,
        First: Container{Width: Length.Percent(100), Height: Length.Percent(100), Padding: 16, BackgroundColor: "#152a36", Children: {Text{Content: "First pane"}}},
        Second: Container{Width: Length.Percent(100), Height: Length.Percent(100), Padding: 16, BackgroundColor: "#202236", Children: {Text{Content: "Second pane"}}},
      }),
    }}
    if ShowArea { root.Children.Add(Cell.Mount[TextAreaInput, TextArea]("area", TextAreaInput{
      Value: Value, OnChange: TextChanged, Label: "Notes", Placeholder: "Write a note", Width: Length.Percent(100), MinimumRows: 4,
      ReadOnly: ReadOnly, Disabled: Disabled, Wrap: NoWrap ? TextWrap.NoWrap : TextWrap.Wrap, ShowFocusHighlight: true, CreateEditor: CaptureEditor,
    })) }
    return root
  }
}
...
func CompositeInteractions() {
  ...
  let window = Window{Title: "Goo Widgets composite verification", Width: 700, Height: 610, Root: host, AccessibilityAdapter: semantics}
  window.Open()
  try {
    ...
    PumpFrames(window, 8)
    Require(host.Selected == "three" && FindSemantics(semantics.Tree!!.Root, "Activity")!!.Focused, "Arrow navigation did not skip the disabled tab")
    host.Manual = true
    host.Rebuild()
    PumpFrames(window, 5)
    SendKey(id, SDLScancode.End)
    PumpFrames(window, 8)
    Require(host.Selected == "three" && FindSemantics(semantics.Tree!!.Root, "Connections")!!.Focused, "Manual tab navigation changed selection or lost focus")
    ...
    PumpFrames(window, 4)
    Require(host.Split == cancelled && host.Commits == 2, "Pointer cancellation retained splitter capture or committed")
    ...
    host.Rebuild()
    PumpFrames(window, 8)
    Require(host.Editor!!.Controller == controller && controller.Selection.Anchor.Offset == 0 && controller.Selection.Active.Offset == 5 && host.Changes == 0,
      "External TextArea value lost controller/selection or emitted a user change")
    controller.Selection = TextSelection{Anchor: TextPosition{Offset: host.Value.Length}, Active: TextPosition{Offset: host.Value.Length}}
    ...
    SendKey(id, SDLScancode.Return)
    PumpFrames(window, 8)
    Require(host.Value.EndsWith("\n") && host.Changes == 1, "Native multiline editing did not emit a controlled string")
    host.ReadOnly = true
    host.Rebuild()
    PumpFrames(window, 5)
    SendKey(id, SDLScancode.Return)
    ...
    Require(host.TabViewport.ScrollRange.X > 0.0 && host.TabRoot!!.Children.Count == 6, "Tab overflow dropped entries or lost its scroll range")
  ...
  Console.WriteLine("PASS: split drag/cancel/resize/keyboard, tab pointer/disabled/manual/automatic/overflow, TextArea controlled edits/selection/ownership/read-only")
}
