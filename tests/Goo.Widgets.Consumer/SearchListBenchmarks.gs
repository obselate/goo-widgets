...
func SearchListBenchmarks() {
  ...
  for i in 0 ... items.Length { items[i] = SelectionItem{Id: i.ToString(), Label: "Item " + i.ToString()} }
  let input = SearchList{Items: items, SelectedId: "0"}
  for i in 0 ... 300 { GC.KeepAlive(input.Build()) }
  ...
  Array.Sort(times)
  Console.WriteLine("searchlist-build: items=1000 warmup=300 samples=2000 allocated_B_per_build="
