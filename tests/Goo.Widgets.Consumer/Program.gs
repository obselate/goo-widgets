...
func PackageComposition() {
    ...
    Require(
        enabled && after.Accessibility?.Checked == AccessibilityChecked.True,
        "Controlled toggle did not reflect host state."
    )
...
func NumericAndIdentityRegressions() {
    ...
    ProgressBar{
        Value: Double.NaN,
        CreateFill: (resolved ProgressBar) -> {
    ...
    Require(
        empty.Children.Count == 2 && (empty.Children[1] as Text)!!.Content == "Nothing found",
...
func MaterialIconRegressions() {
    ...
    for name in names {
        ...
        Require(
            icon.Path.ViewBoxWidth > 0.0 && icon.Path.ViewBoxHeight > 0.0,
    ...
    for state in[]AccessibilityChecked{AccessibilityChecked.True, AccessibilityChecked.Mixed} {
        ...
        Require(
            checkbox.Children.Count == 1 && checkbox.Children[0] is Shape,
    ...
    for maximized in[]bool{false, true} {
        ...
        Require(
            row.Children[2].Accessibility?.Name == if maximized {
                "Restore"
            } else {
                "Maximize"
            },
...
func Main() {
    ...
    if Environment.GetEnvironmentVariable("GOO_WIDGETS_CHROME") == "1" {
        ChromeInteractions()
        return
    }
    if Environment.GetEnvironmentVariable("GOO_WIDGETS_MARKDOWN") == "1" {
        MarkdownInteractions()
        return
    }
    if Environment.GetEnvironmentVariable("GOO_WIDGETS_CALENDAR") == "1" {
        CalendarInteractions()
        return
    }
    if Environment.GetEnvironmentVariable("GOO_WIDGETS_COMBOBOX") == "1" {
        ComboBoxInteractions()
        return
    }
    if Environment.GetEnvironmentVariable("GOO_WIDGETS_OVERLAYS") == "1" {
        OverlayInteractions()
        return
    }
    if Environment.GetEnvironmentVariable("GOO_WIDGETS_DATA_GRID") == "1" {
    ...
    if Environment.GetEnvironmentVariable("GOO_WIDGETS_TIME_AXIS") == "1" {
        TimeAxisInteractions()
        return
    }
    if Environment.GetEnvironmentVariable("GOO_WIDGETS_GRID_CHARTS") == "1" {
        GridChartInteractions()
        return
    }
    if Environment.GetEnvironmentVariable("GOO_WIDGETS_COMPOSITES") == "1" {
        CompositeInteractions()
        return
    }
    if Environment.GetEnvironmentVariable("GOO_WIDGETS_SEARCHLIST_BENCH") == "1" {
        SearchListBenchmarks()
        return
    }
    if Environment.GetEnvironmentVariable("GOO_WIDGETS_SEARCHLIST") == "1" {
        SearchListInteractions()
        return
    }
    if Environment.GetEnvironmentVariable("GOO_WIDGETS_DISCLOSURE") == "1" {
    ...
    SliderInteractions()
    WidgetErgonomics()
    if Environment.GetEnvironmentVariable("GOO_WIDGETS_WINDOW") == "1" {
        WindowInteractions()
    }
    Console.WriteLine(
        "PASS: packaged G# composition, icons, copy-update, callbacks, numeric and virtual identity regressions."
    )
}
