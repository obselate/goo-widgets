...
func TimeAxisContracts() {
  ...
  try {
    probe.Render(TimeAxisInput{Start: day, End: day.AddHours(12), Events: AxisEvents(), MaximumLanes: 2,
      OnOverflow: (items []TimeAxisEvent) -> { overflowActivated = items.Length },
      CreateEvent: (input TimeAxisInput, placement TimeAxisPlacement, prepared Button) -> {
        placements.Add(placement.Event.Id!!, placement)
        return Button{Children: {Text{Content: placement.Event.Label!!}}}
      },
      CreateOverflow: (input TimeAxisInput, items []TimeAxisEvent, prepared Button) -> { hidden = items
        return prepared },
      CreateRoot: (input TimeAxisInput, prepared Container) -> { root = prepared
        return prepared },
    })
    Require(placements.Count == 5 && !placements.ContainsKey("outside") && hidden.Length == 1 && hidden[0].Id == "overflow", "TimeAxis lost clipping or collision overflow")
    Require(placements["alpha"].Lane == 0 && placements["beta"].Lane == 1 && placements["gamma"].Lane == 1,
      "Adjacent TimeAxis events did not reuse collision lanes")
    Require(placements["point"].Width == 6.0 && placements["target"].Lane != placements["point"].Lane, "Point width was omitted from lane collisions")
    let canvas = (root!!.Children[0] as Container)!!
    for child in canvas.Children { if child.Key == "overflow" { child.OnClick?.Invoke() } }
    Require(overflowActivated == 1 && root!!.Handle != nil, "Overflow action or prepared viewport handle was lost")
    var rejected bool
    try { probe.Render(TimeAxisInput{Start: day, End: day.AddDays(1), TickInterval: TimeSpan.FromSeconds(1)}) }
    catch (error ArgumentException) { rejected = true }
    Require(rejected, "TimeAxis accepted unbounded tick creation")
    rejected = false
    try { probe.Render(TimeAxisInput{Start: day, End: day}) }
    catch (error ArgumentException) { rejected = true }
...
internal class TimeAxisHost : Cell {
  ...
  private func Event(input TimeAxisInput, placement TimeAxisPlacement, prepared Button) Button {
    ...
    if placement.Event.Id == "point" { prepared.BackgroundColor = "#fbbf24" }
    Buttons[placement.Event.Id!!] = prepared
    return prepared
  }
  private func CaptureRoot(input TimeAxisInput, prepared Container) Container { Root = prepared
    return prepared }
  public override func Build() Blob {
    let root = Container{Width: Length.Percent(100), Height: Length.Percent(100), Padding: 24, Gap: 20, BackgroundColor: "#111318",
      Children: {Text{Key: "title", Content: "TimeAxis · September 14", FontSize: 24, Color: "#fafafa"},
        Text{Key: "hint", Content: "Scroll the ruler or use Left/Right, Home/End. Overlapping events occupy separate lanes.", Color: "#a1a1aa"}}}
    if Show {
      root.Children.Add(Cell.Mount[TimeAxisInput, TimeAxis]("axis", TimeAxisInput{
        Start: AxisDay(), End: AxisDay().AddHours(12), Events: AxisEvents(), MaximumLanes: 2, CurrentTime: Current,
        PixelsPerHour: Scale, SelectedId: Selected ?? "", InitialEventId: "target", CreateEvent: Event, CreateRoot: CaptureRoot,
        OnSelect: (id string) -> { Selected = id
          Rebuild() },
        OnOverflow: (items []TimeAxisEvent) -> { OverflowClicks++ },
        OnViewportChanged: (view TimeAxisViewport) -> { ViewChanges++
          LastView = view },
      }))
    }
    return root
...
func TimeAxisInteractions() {
  ...
  let window = Window{Title: "TimeAxis verification", Width: 700, Height: 360, Root: host, AccessibilityAdapter: semantics}
  window.Open()
  try {
    ...
    Require(Math.Abs(handle.ScrollOffset.X - expected) < .5 && host.ViewChanges == 0, "Initial event centering failed or was reported as a later scroll")
    ...
    Require(handle.ScrollOffset.X < .1 && host.ViewChanges > 0 && host.LastView.Start == AxisDay(), "TimeAxis failed to report keyboard scrolling")
    ...
    host.Rebuild()
    PumpFrames(window, 10)
    Require(Math.Abs(handle.ScrollOffset.X - 300.0) < .5 && handle.ScrollRange.X > 1400.0, "Scale change lost the viewport or failed to change the scroll range")
    window.Width = 420
    PumpFrames(window, 10)
    ...
    PumpFrames(window, 5) }
  Console.WriteLine("PASS: TimeAxis collision/overflow/point/clipping, native selection, initial centering, user/host scroll, marker/scale/resize and removal")
}
