...
func ComboBoxContracts() {
  ...
  var rejected bool
  try { probe.Render(ComboBoxInput{Items: []ComboBoxOption{ComboBoxOption{Id: "a"}, ComboBoxOption{Id: "a"}}}) }
  catch (error ArgumentException) { rejected = true }
  Require(rejected, "ComboBox accepted duplicate IDs")
  rejected = false
  try { probe.Render(ComboBoxInput{Width: Double.NaN}) } catch (error ArgumentException) { rejected = true }
  ...
  Require(trigger.Disabled && trigger.Accessibility!!.Role == AccessibilityRole.ComboBox && trigger.Accessibility!!.Value == "Pick a target",
...
internal class ComboBoxHost : Cell {
  ...
  private func Open(value bool) { IsOpen = value
    Rebuild() }
  private func Select(value string) { Selected = value
    Commits++
    Rebuild() }
  private func QueryChanged(value string) { Query = value
    Rebuild() }
  private func CaptureTrigger(input ComboBoxInput, prepared Button) Button { Trigger = prepared
    return prepared }
  private func CaptureSearch(input ComboBoxInput, prepared TextEntry) TextEntry { Search = prepared
    return prepared }
  private func CapturePanel(input ComboBoxInput, prepared Container) Container { Panel = prepared
    return prepared }
  private func CaptureRow(input ComboBoxInput, item ComboBoxOption, prepared Container) Container { Rows[item.Id!!] = prepared
    return prepared }
  private func Options() []ComboBoxOption {
    if Empty { return []ComboBoxOption{} }
    if !Large { return []ComboBoxOption{ComboBoxOption{Id: "alpha", Label: "Alpha"}, ComboBoxOption{Id: "locked", Label: "Locked", Disabled: true},
      ComboBoxOption{Id: "beta", Label: "Beta"}, ComboBoxOption{Id: "gamma", Label: "Gamma", Content: Text{Content: "Gamma · custom", Color: "#86efac"}}} }
    let items = [1500]ComboBoxOption
    for index in 0 ... items.Length { items[index] = ComboBoxOption{Id: "item-" + index.ToString(), Label: "Option " + index.ToString("D4"), Disabled: index % 100 == 0} }
    return items
  }
  public override func Build() Blob {
    let root = Container{Handle: Overlay, Width: Length.Percent(100), Height: Length.Percent(100), Padding: 24.0, Gap: 16.0,
      BackgroundColor: "#111318", Color: "#fafafa", Children: {
        ...
        Text{Key: "hint", Content: "Open, navigate, cancel or commit. Search narrows 1,500 virtual options.", Color: "#a1a1aa"},
        Button{Key: "before", Handle: Before, Height: 36.0, Width: 160.0, BackgroundColor: "#27272a",
          OnClick: () -> { BackgroundClicks++ }, Children: {Text{Content: "Background action"}}},
      }}
    if Show { root.Children.Add(Cell.Mount[ComboBoxInput, ComboBox]("combo", ComboBoxInput{
      OverlayHost: Overlay, Width: 320.0, PopupHeight: 250.0, SelectedId: Selected, OnSelect: Select,
      Items: Options(), Open: IsOpen, OnOpenChange: Open, Query: Query, OnQueryChange: QueryChanged,
      Searchable: Searchable, Disabled: Disabled, AccessibilityName: "Target selector", CreateTrigger: CaptureTrigger,
      CreateRow: CaptureRow, CreateSearch: CaptureSearch, CreatePopup: CapturePanel,
    })) }
    return root
  }
}
...
func SelectorFindRole(node AccessibilityNode?, role AccessibilityRole) AccessibilityNode? {
  ...
  for child in node.Children { if let found = SelectorFindRole(child, role) { return found } }
  return nil
}
func SelectorCountRole(node AccessibilityNode?, role AccessibilityRole) int32 {
  guard let node = node else { return 0 }
  var count = node.Role == role ? 1 : 0
  for child in node.Children { count += SelectorCountRole(child, role) }
  return count
}
...
unsafe func SelectorType(window Window, id uint32, text string) {
  Require(Marshal.OffsetOf[SelectorTextEvent]("Text") == Marshal.OffsetOf[SDLTextInputEvent]("Text"), "SDL text layout differs")
...
func ComboActive(adapter SearchListSemantics, name string) bool {
  ...
  return option != nil && ((entry?.Relationships.ActiveDescendant == option.Id) || (list?.Relationships.ActiveDescendant == option.Id))
}
func ComboBoxInteractions() {
  ComboBoxContracts()
  let host = ComboBoxHost()
  let semantics = SearchListSemantics()
  let window = Window{Title: "ComboBox verification", Width: 720, Height: 520, Root: host, AccessibilityAdapter: semantics}
  window.Open()
  try {
    ...
    PumpFrames(window, 7)
    Require(ComboActive(semantics, "Alpha") && host.Selected == "beta", "Arrow navigation selected or failed to skip a disabled option")
    SendKey(id, SDLScancode.Escape)
    PumpFrames(window, 10)
    Require(!host.IsOpen && host.Commits == 0 && OverlayFocus(semantics, "Target selector"), "ComboBox cancellation changed selection or lost focus")
    ...
    PumpFrames(window, 10)
    Require(!host.IsOpen && host.Selected == "gamma" && host.Commits == 1 && OverlayFocus(semantics, "Target selector"), "ComboBox commit/restore failed")
    CompositeClick(window, host.Trigger!!.Handle!!)
    PumpFrames(window, 12)
    Require(!window.PerformAccessibilityAction(FindSemantics(semantics.Tree?.Root, "Locked")!!.Id, AccessibilityActionRequest(AccessibilityAction.Select)), "Disabled option accepted an accessibility selection")
    ...
    Require(ComboActive(semantics, "Option 1201") && (host.Search?.Handle?.IsMounted ?? false), "Long searchable list did not reveal its selected option")
    Require(SelectorCountRole(semantics.Tree?.Root, AccessibilityRole.ListItem) < 30, "ComboBox realized its full long list")
    ...
    PumpFrames(window, 5) }
  Console.WriteLine("PASS: native ComboBox controlled/disabled/custom options, keyboard commit/cancel, focus restoration, long-list reveal, search, empty input, resize and unmount")
}
