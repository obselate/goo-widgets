...
internal open class NativePickerProbe : Cell[ColorPickerInput], IDisposable {
  ...
  public func Dispose() { picker.Dispose() }
  ...
  protected override func Build(input ColorPickerInput) Blob {
    ...
    if !input.Compact {
      ...
      for mode in modes.Children { mode.Handle = ElementHandle{} }
    }
    LastRoot = root
    return root
  }
...
internal class ErgonomicHost : Cell {
  ...
  public override func Build() Blob {
    let checkbox = (Checkbox{
      Label: "Enable preview", State: Checked, Disabled: Disabled,
      OnChange: (value AccessibilityChecked) -> { Checked = value },
    }.Build() as Button)!!
    ...
    return Container{
      Width: Length.Percent(100), Height: Length.Percent(100), Padding: 24.0, Gap: 16.0,
      BackgroundColor: "#18181b",
      Children: {
        checkbox,
        Cell.Mount[SliderInput, Slider]("labeled-slider", SliderInput{
          Label: "Opacity", ShowValue: true, Value: Value, Width: Length.Percent(50),
          FormatValue: (value float64) -> (value * 100.0).ToString("0") + "%",
          OnValueChanged: (value float64) -> { Value = value
            Rebuild() },
          CreateRoot: (input SliderInput, root Container) -> { SliderRoot = root
            return root },
        }),
        Cell.Mount[ColorPickerInput, NativePickerProbe]("full-picker", ColorPickerInput{
          Value: Rgb, Mode: Mode, WheelSize: 200.0, Compact: Compact, Disabled: Disabled,
          OnValueChanged: (value int32) -> { Rgb = value
            Rebuild() },
          OnModeChanged: (mode ColorMode) -> { ModeChanges++ },
        }),
      },
    }
  }
...
func ErgonomicWindowInteractions() {
  ...
  try {
    ...
    Require(host.CheckboxRoot!!.Accessibility?.Checked == AccessibilityChecked.True, "Checkbox label click did not rebuild its checked state.")
    let slider = host.SliderRoot!!
    Require(Math.Abs(slider.Handle!!.BorderBox.Width - (float64(window.Width) - 48.0) * 0.5) < 1.0,
    ...
    PumpFrames(window, 4)
    Require(host.Value == 1.0 && host.SliderRoot!!.Accessibility?.Range?.Text == "100%",
    ...
    Require(host.ModeChanges == 1 && NativeModeButton(1).Accessibility?.Selected == true,
    ...
    host.Rebuild()
    PumpFrames(window, 4)
    Require(NativeModeButton(1).Accessibility?.Selected == true, "External RGB update reset the selected color mode.")
    let preview = (NativePickerProbe.LastRoot!!.Children[3] as Container)!!
    Require((preview.Children[1] as Text)!!.Content == "#CC8844", "Full picker preview did not follow the host color.")
    host.Mode = ColorMode.Hsl
    host.Rebuild()
    PumpFrames(window, 4)
    Require(NativeModeButton(0).Accessibility?.Selected == true, "External mode update did not reach the full picker.")
    host.Disabled = true
    host.Rebuild()
    PumpFrames(window, 4)
    ClickNativeButton(window, windowId, NativeModeButton(2))
    ClickNativeButton(window, windowId, host.CheckboxRoot!!.Children[1])
    Require(host.ModeChanges == 1 && host.Checked == AccessibilityChecked.True,
    ...
    Require(compactWheel.Handle!!.IsMounted && compactWheel.Handle!!.Focus(), "Compact picker lost the mounted wheel.")
    ...
    host.Rebuild()
    PumpFrames(window, 4)
    Require(NativeModeButton(0).Accessibility?.Selected == true, "Returning to full composition lost the color mode.")
    Console.WriteLine("PASS: native label click, slider width/focus/value, color modes/preview, disabled input, compact/full transition.")
  } finally {
    window.RequestClose()
    PumpFrames(window, 3)
  }
