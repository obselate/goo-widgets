...
func MarkdownContracts() {
  ...
  var rejected bool
  try { probe.Render(MarkdownViewInput{FontSize: Double.NaN}) } catch (error ArgumentException) { rejected = true }
  ...
  let input = MarkdownViewInput{Text: "# Same\n\n# Same\n\n# Same-2\n\n---\n\n> [!WARNING]\n> Check this\n\n| A | B |\n| - | - |\n| x | y |\n\n```gs\ntext\n```\n\n3. first\n4. second",
    OnHeadings: (value IReadOnlyList[MarkdownHeading]) -> { headings = value },
    CreateBlock: (block MarkdownBlock, prepared Blob) -> { kinds.Add(block.Kind)
      return prepared }}
  probe.Render(input)
  Require(headings != nil && headings!!.Count == 3 && headings!! [0].Id == "same" && headings!! [1].Id == "same-2"
      && headings!! [2].Id == "same-2-2", "Markdown heading IDs were not unique")
  for kind in []MarkdownBlockKind {MarkdownBlockKind.Heading, MarkdownBlockKind.Alert, MarkdownBlockKind.Table, MarkdownBlockKind.Code, MarkdownBlockKind.List, MarkdownBlockKind.Rule} {
...
internal class MarkdownHost : Cell {
  ...
  private func Open(target string) { LastLink = target
    Links++ }
  private func HeadingsChanged(value IReadOnlyList[MarkdownHeading]) { Headings = value
    HeadingCalls++ }
  ...
  private func Style(run MarkdownInline, prepared Style) Style { StylePasses++
    return prepared }
  private func Inline(run MarkdownInline, prepared Blob) Blob { InlinePasses++
    if run.Kind == MarkdownInlineKind.Link { prepared.Handle = ElementHandle()
  ...
  private func Resolve(run MarkdownInline) Blob? {
    ...
    Images++
    return Container{Width: 240.0, Height: 56.0, BorderRadius: 6.0, BackgroundColor: "#312e81", AlignItems: AlignItems.Center, JustifyContent: JustifyContent.Center,
      Children: {Text{Content: "Host-resolved diagram", Color: "#c7d2fe", FontSize: 14.0}}}
  }
  internal func Text() string -> "# Deployment notes\n\n"
  +(Changed ? "Updated paragraph. " : "Select this paragraph. ")
  +"It contains **bold**, *italic*, ~~old~~, `inline code`, and the [" + (LongLink ? MarkdownLongCaption() : "release guide")
  ...
  public override func Build() Blob {
    let root = Container{Width: Length.Percent(100), Height: Length.Percent(100), Padding: 20.0, Gap: 12.0,
      BackgroundColor: "#111318", Color: "#fafafa", Children: {
        Button{Key: "before", Handle: Before, Height: 32.0, Width: 190.0, BackgroundColor: "#27272a", Children: {Text{Content: "Markdown verification"}}},
      }}
    if Show { root.Children.Add(Container{Key: "scroll", Handle: Viewport, FlexGrow: 1.0, FlexBasis: 0.0, MinHeight: 0.0, OverflowY: Overflow.Scroll, Children: {
      Cell.Mount[MarkdownViewInput, MarkdownView]("view", MarkdownViewInput{Text: Text(), FontSize: FontSize,
        OnLink: Open, ResolveImage: Resolve, OnHeadings: HeadingsChanged, CreateEditor: Editor, CreateInline: Inline, CreateInlineStyle: Style}),
    }}) }
    return root
  }
}
...
func MarkdownInteractions() {
  ...
  let window = Window{Title: "Markdown verification", Width: 820, Height: 860, Root: host, AccessibilityAdapter: semantics}
  window.Open()
  try {
    PumpFrames(window, 25)
    let id = OnlyNativeWindow()
    Require(host.Headings.Count == 4 && host.Headings[3].Id == "details-2" && host.Headings[0].Handle!!.IsMounted, "Markdown headings did not mount")
    Require(SelectorCountRole(semantics.Tree?.Root, AccessibilityRole.Heading) == 4 && SelectorCountRole(semantics.Tree?.Root, AccessibilityRole.Link) == 1,
      "Markdown heading/link accessibility roles are missing")
    Require(host.Images > 0 && host.InlinePasses > 0 && host.StylePasses > 0, "Markdown skipped customization hooks")
    ...
    PumpFrames(window, 7)
    Require(window.GetClipboardText() == controller.Document.GetText(), "Markdown keyboard copy did not preserve plain text")
    Require(!window.GetClipboardText().Contains("**") && window.GetClipboardText().Contains("release guide"), "Markdown copy leaked markup or dropped link text")
    ...
    Require(SelectorCountRole(semantics.Tree?.Root, AccessibilityRole.ColumnHeader) == 2
        && SelectorCountRole(semantics.Tree?.Root, AccessibilityRole.GridCell) == 4, "Markdown table added a phantom trailing column")
    let link = FindSemantics(semantics.Tree?.Root, "release guide")!!
    Require(window.PerformAccessibilityAction(link.Id, AccessibilityActionRequest(AccessibilityAction.Focus)), "Markdown link could not receive accessible focus")
    ...
    Require(window.PerformAccessibilityAction(link.Id, AccessibilityActionRequest(AccessibilityAction.Activate)), "Markdown accessible activation failed")
    PumpFrames(window, 6)
    ...
    Require(Object.ReferenceEquals(narrow.Controller, controller) && narrow.Handle!!.BorderBox.Height > height + 10.0,
    ...
    Require(host.Editors["root/1"].Handle!!.BorderBox.Height > narrowHeight && controller.Selection.Anchor.Offset != controller.Selection.Active.Offset,
      "Markdown font change failed measurement or lost selection")
    CaptureIssueProof(window, "markdown-narrow")
    Require(host.Headings[3].Handle!!.ScrollIntoView(), "Markdown heading handle failed table-of-contents navigation")
    ...
    host.Rebuild()
    PumpFrames(window, 20)
    Require(ControllerDisposed(controller) && host.Editors["root/1"].Controller.Document.GetText().StartsWith("Updated paragraph."),
    ...
    Require(longLink.Bounds.Width <= host.Viewport.ContentBox.Width + .2 && longLink.Bounds.Height > 30.0,
      "Markdown long link did not wrap to the measured paragraph width")
    Require(longEditor.Handle!!.TryGetTextCaretRect(TextPosition{Offset: nextText, Affinity: TextAffinity.Downstream}, TextCoordinateSpace.Window, out caret)
        && caret.Y >= longLink.Bounds.Y + longLink.Bounds.Height - .2, "Wrapped link overlapped the following text")
    ...
    PumpFrames(window, 5) }
  Console.WriteLine("PASS: native Markdown parsing, intrinsic resize/font measurement, keyboard copy/read-only selection, accessible links/headings, host images, ToC, content refresh and disposal")
}
