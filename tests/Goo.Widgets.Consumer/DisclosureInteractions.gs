...
internal class DisclosureChild : Cell, IDisposable {
  ...
  public override func Build() Blob {
    let button = Goo.Button{
      Handle: Handle, Height: 40, OnClick: () -> { Count++ },
      Accessibility: Accessibility{Role: AccessibilityRole.Button, Name: "Retained child"},
      Children: {Text{Content: "Retained counter: " + Count.ToString()}},
    }
    Button = button
    return button
  }
...
internal class DisclosureHost : Cell {
  ...
  func MakeIndicator(props Disclosure) Blob -> Text {Content: if props.Expanded { "−" } else { "+" }}
  ...
  func MakeRoot(props Disclosure, header Button, body Container) Container -> Container {
    Width: props.Width!!, BorderRadius: 8, BorderWidth: 1,
    BorderColor: "#28765c", BackgroundColor: "#102822", Children: {header, body},
  }

  public override func Build() Blob {
    if !Show { return Container{} }
    let section = Disclosure{
      Label: "Connection details", Expanded: Expanded, Disabled: Disabled,
      ShowFocusHighlight: true, HeaderPadding: 0.0,
      Content: Cell.Mount[DisclosureChild]("child", (child DisclosureChild) -> {
        if Child != child { Mounts++ }
        Child = child
      }),
      OnExpandedChange: (expanded bool) -> { Expanded = expanded
        Changes++
        Rebuild() },
      CreateIndicator: MakeIndicator,
      CreateHeader: (headerProps Disclosure, header Button) -> {
        Require(headerProps.HeaderPadding == 0.0 && headerProps.BackgroundColor != nil,
    ...
    return Container{Padding: 24, BackgroundColor: "#09090b", Color: "#fafafa", Gap: 16, Children: {
...
func DisclosureInteractions() {
  ...
  try {
    ...
    Require(child.Handle.IsMounted && child.Disposals == 0 && !child.Handle.Focus(),
      "Collapsed content must stay mounted while rejecting focus.")
    Require(FindSemantics(semantics.Tree?.Root, "Retained child") == nil
    ...
    PumpFrames(window, 4)
    Require(host.Expanded && host.Changes == 2 && host.Child == child && child.Count == 1 && host.Mounts == 1,
    ...
    Require(host.Expanded && host.Changes == 4 && !host.HeaderHandle.Focus()
    ...
    host.Rebuild()
    PumpFrames(window, 4)
    Require(child.Disposals == 1 && !child.Handle.IsMounted, "Removing disclosure did not dispose its child exactly once.")
    Console.WriteLine("PASS: Disclosure pointer/Enter/Space, disabled input, hidden accessibility, retained child state, factories, unmount disposal.")
  } finally {
    window.RequestClose()
    PumpFrames(window, 3)
  }
