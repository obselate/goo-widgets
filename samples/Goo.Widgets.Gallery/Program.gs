...
func VerifyGallery(registry GalleryRegistry) {
    ...
    if window.IsOpen {
        throw InvalidOperationException("Gallery window did not close.")
    }
}
...
func GooCliExecutable() string {
    ...
    let toolName = if OperatingSystem.IsWindows() {
        "goo.exe"
    } else {
        "goo"
    }
...
func CaptureGallery(registry GalleryRegistry, directory string, requestedPage string?) {
    ...
    let first = if requestedPage != nil {
        registry.IndexOf(requestedPage)
    } else {
        0
    }
    let end = if requestedPage != nil {
        first + 1
    } else {
        registry.Count()
    }
    ...
    if window.IsOpen {
        throw InvalidOperationException("Gallery screenshot window did not close.")
    }
}
...
func SpotlightWidth(page GalleryPage) int32 {
    if page.Title() == "Color picker" {
        return 600
    }
    if page.Title() == "Checkbox" {
        return 640
    }
...
func CaptureSpotlight(registry GalleryRegistry, directory string, requestedPage string) {
    ...
    if window.IsOpen {
        throw InvalidOperationException("Spotlight screenshot window did not close.")
    }
}
