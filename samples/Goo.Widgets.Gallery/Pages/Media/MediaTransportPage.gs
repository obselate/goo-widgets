...
internal open class MediaTransportExample : Cell {
  ...
  public override func Build() Blob -> Container {
    ...
    Gap: 12.0,
    Children: {
      Text{ Key: "media-title", Content: "Example item " + item.ToString(), FontSize: 16.0, FontWeight: 700, Color: "#fafafa" },
      Cell.Mount[MediaTransportInput, MediaTransport]("media-transport", MediaTransportInput{
        Position: position,
        Duration: 245.0,
        Volume: volume,
        Playing: playing,
        OnPrevious: () -> { item = Math.Max(1, item - 1)
          position = 0.0
          status = "Previous"
          Rebuild() },
        ...
        OnNext: () -> { item++
          ...
          status = "Next"
          Rebuild() },
        OnSeekChanged: (value float64) -> { position = value
          status = "Seeking"
          Rebuild() },
        OnSeekCommitted: (value float64) -> { position = value
          status = "Seek committed"
          Rebuild() },
        OnVolumeChanged: (value float64) -> { volume = value
          status = "Volume preview"
          Rebuild() },
        OnVolumeCommitted: (value float64) -> { volume = value
