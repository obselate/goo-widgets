...
internal class MenuExample : Cell {
  ...
  private func Dismiss() { isOpen = false
    Rebuild() }
  private func Activate(id string) { selected = "Selected: " + id
    Rebuild() }
  private func OpenTrigger() { usePoint = false
    isOpen = true
    Rebuild() }
    ...
    var menu = MenuInput{Open: isOpen, Placement: openUp ? PopoverPlacement.TopStart : PopoverPlacement.BottomStart,
      ...
      Items: []MenuItem{
        ...
        MenuItem{Id: "tools", Label: "Tools", Children: []MenuItem{
          MenuItem{Id: "copy", Label: "Copy address"}, MenuItem{Id: "reveal", Label: "Reveal in folder"},
          MenuItem{Id: "more", Label: "More", Children: []MenuItem{MenuItem{Id: "inspect", Label: "Inspect details"}}},
        }}, MenuItem{Id: "offline", Label: "Sync unavailable", Disabled: true},
      }}
    if usePoint { menu.WindowPoint = point } else { menu.Anchor = trigger }
    return Container{Width: 720.0, Height: 420.0, Gap: 14.0, Color: "#fafafa",
      OnPointerDown: (event PointerEvent) -> {
        if event.Button == PointerButton.Secondary { event.PreventDefault()
        ...
        Button{Key: "trigger", Handle: trigger, Position: PositionType.Absolute, Left: 350.0, Top: 270.0,
          ...
          OnKeyDown: (event KeyEvent) -> { if event.Key == Key.Menu || event.Key == Key.F10 && event.Modifiers.Shift {
            ...
            OpenTrigger() } }, Children: {Text{Content: "Open actions"}}},
