...
public open class ColorPicker : Cell[ColorPickerInput], IDisposable {
    ...
    protected override func Build(input ColorPickerInput) Blob {
        ...
        let slider = Cell.Mount[SliderInput, Slider](
            "color-tone",
            SliderInput{
                Value: tone,
                Minimum: 0.0,
                Maximum: 1.0,
                Step: 0.01,
                AccessibilityName: if currentMode == ColorMode.Hsv {
                    "Color value"
                } else {
                    "Color lightness"
                },
    ...
    private func Resolve(input ColorPickerInput) ColorPickerInput {
        if input.Value < 0 || input.Value > 0xFFFFFF {
            throw ArgumentOutOfRangeException("Value")
        }
        let wheelSize = if input.WheelSize == 0.0 {
            240.0
        } else {
            input.WheelSize
        }
        let markerSize = if input.MarkerSize == 0.0 {
            22.0
        } else {
            input.MarkerSize
        }
        let sliderHeight = if input.ToneSliderHeight == 0.0 {
            28.0
        } else {
            input.ToneSliderHeight
        }
        if !Double.IsFinite(wheelSize) || wheelSize <= 0.0 {
            throw ArgumentOutOfRangeException("WheelSize")
        }
        if !Double.IsFinite(markerSize) || markerSize <= 0.0 {
            throw ArgumentOutOfRangeException("MarkerSize")
        }
        if !Double.IsFinite(sliderHeight) || sliderHeight <= 0.0 {
            throw ArgumentOutOfRangeException("ToneSliderHeight")
        }
        return input with{
    ...
    private func BuildWheel() Blob {
        ...
        let valueText = Math.Round(hue).ToString(CultureInfo.InvariantCulture) + "," + Math.Round(radius * 100.0)
    ...
    private func SelectMode(mode ColorMode) {
        if resolved.Disabled || mode == currentMode {
            return
        }
        currentMode = mode
        ReadCoordinates(currentRgb)
    ...
    private func ReadCoordinates(rgb int32) {
        if currentMode == ColorMode.Hsl {
            ...
            radius = if maximum <= 0.000001 {
                0.0
            } else {
                Math.Clamp(color.Chroma / maximum, 0.0, 1.0)
            }
        }
    }
    ...
    private func UpdateColor(commit bool) {
        ...
        if commit {
            resolved.OnValueCommitted?.Invoke(currentRgb)
        }
    }
    ...
    private func CoordinatesFromPointer(e PointerEvent) {
        ...
        if nextRadius >= 0.000001 {
            hue = WrapHue(Math.Atan2(y, x) * 180.0 / Math.PI)
        }
        radius = nextRadius
        UpdateColor(false)
    }
    ...
    private func HandleAccessibility(request AccessibilityActionRequest) bool {
        ...
        if !Double.TryParse(parts[0], NumberStyles.Float, CultureInfo.InvariantCulture, out var nextHue) ||
            !Double.IsFinite(nextHue) {
            return false
        }
        if !Double.TryParse(parts[1], NumberStyles.Float, CultureInfo.InvariantCulture, out var percentage) ||
