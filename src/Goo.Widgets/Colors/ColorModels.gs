...
public enum ColorMode { Hsl; Hsv; Oklch }
...
public data struct HslColor {
  ...
  shared {
    ...
    public func FromRgb(rgb int32) HslColor {
      ...
      return HslColor{
        Hue: WrapHue(hue),
        Saturation: if delta == 0.0 { 0.0 } else { delta / (1.0 - Math.Abs(2.0 * lightness - 1.0)) },
        Lightness: lightness,
      }
    }
  }
...
public data struct OklchColor {
  ...
  /// Converts this value to packed 24-bit sRGB.
  public func Rgb() int32 -> Encode(Lightness, Chroma * Math.Cos(Hue * Math.PI / 180.0), Chroma * Math.Sin(Hue * Math.PI / 180.0))
  ...
  shared {
    ...
    public func FromRgb(rgb int32) OklchColor {
      ...
      return OklchColor{
        ...
        Chroma: if chroma < 0.000001 { 0.0 } else { chroma },
    ...
    private func Channel(value float64) int32 {
      ...
      return int32(Math.Round(255.0 * if clamped <= 0.0031308 { 12.92 * clamped } else { 1.055 * Math.Pow(clamped, 1.0 / 2.4) - 0.055 }))
    }

    private func Channels(lightness float64, a float64, b float64, out red float64, out green float64, out blue float64) {
