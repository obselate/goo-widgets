...
public data struct IconButton {
  ...
  public func Build() Blob {
    ...
    let width = if Width == 0.0 { 40.0 } else { Width }
    let height = if Height == 0.0 { width } else { Height }
    ...
    return Button{
      ...
      BackgroundColor: if resolved.Active { resolved.ActiveBackgroundColor!! } else { resolved.BackgroundColor!! },
      Hover: Style{ BackgroundColor: resolved.HoverBackgroundColor!! },
      Focus: if resolved.ShowFocusHighlight { Style{ OutlineWidth: 2.0, OutlineColor: resolved.FocusOutlineColor!!, OutlineOffset: 2.0 } } else { Style{} },
