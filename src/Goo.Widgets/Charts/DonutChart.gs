...
public data struct DonutChart {
  ...
  public func Build() Blob {
    let resolved = this with{Size = Size ?? 160.0, Thickness = Thickness ?? 24.0, TrackColor = TrackColor ?? Color.Parse("#27272a"), AccessibilityName = AccessibilityName ?? "Chart"}
    let size = resolved.Size!!
    let thickness = resolved.Thickness!!
    if !Double.IsFinite(size) || size <= 0.0 || !Double.IsFinite(thickness) || thickness <= 0.0 || thickness > size / 2.0 { throw ArgumentOutOfRangeException("Donut size/thickness") }
    ...
    plot.Children.Add(Shape{Key: "track", Width: size, Height: size, Position: PositionType.Absolute, Left: 0, Top: 0,
      Path: Ring(-Math.PI / 2.0, 1.0, thickness / size), BackgroundColor: resolved.TrackColor!!, Accessibility: Accessibility{Hidden: true}})
    var angle = -Math.PI / 2.0
    for segment in segments {
      if segment.Fraction <= 0.0 { continue }
      let path = Ring(angle, segment.Fraction, thickness / size)
      var shape = Shape{Path: path, BackgroundColor: segment.Series.Color!!, Focus: Style{Opacity: .7}, Cursor: OnActivate == nil ? Cursor.Default : Cursor.Pointer}
      if let create = CreateSlice { shape = create(resolved, segment, shape) }
      shape.Path = path
      shape.Width = size
      shape.Height = size
    ...
    if let center = Center {
      ...
      plot.Children.Add(Container{Key: "center", Position: PositionType.Absolute, Left: (size - side) / 2.0, Top: (size - side) / 2.0,
        Width: side, Height: side, AlignItems: AlignItems.Center, JustifyContent: JustifyContent.Center, Overflow: Overflow.Hidden, HitTestSelf: false, Children: {center}})
    }
    var root = Container{Gap: 14, AlignItems: AlignItems.Center}
    if let create = CreateRoot { root = create(resolved, root) }
    root.Accessibility = Accessibility{Role: AccessibilityRole.Group, Name: resolved.AccessibilityName!!, Description: ChartParts.Summary(segments)}
    root.Children.Clear()
    root.Children.Add(plot)
    if ShowLegend {
      var legend = ChartParts.Legend(segments, CreateLabel)
      if let create = CreateLegend { legend = create(resolved, legend) }
      legend.Key = "legend"
  ...
  private func Ring(start float64, fraction float64, thickness float64) VectorPath {
    ...
    if full { path.ArcTo(outer, outer, 0, false, true, 50.0 + Math.Cos(start + Math.PI) * outer, 50.0 + Math.Sin(start + Math.PI) * outer) }
    path.ArcTo(outer, outer, 0, !full && fraction > .5, true, 50.0 + Math.Cos(end) * outer, 50.0 + Math.Sin(end) * outer)
    path.LineTo(50.0 + Math.Cos(end) * inner, 50.0 + Math.Sin(end) * inner)
    if inner > 0.0 {
      if full { path.ArcTo(inner, inner, 0, false, false, 50.0 + Math.Cos(start + Math.PI) * inner, 50.0 + Math.Sin(start + Math.PI) * inner) }
      path.ArcTo(inner, inner, 0, !full && fraction > .5, false, 50.0 + Math.Cos(start) * inner, 50.0 + Math.Sin(start) * inner)
    }
    path.Close()
    return path.Build()
  }
