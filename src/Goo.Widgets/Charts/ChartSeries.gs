        ...
        internal func Resolve(series[]ChartSeries)[]ChartSegment {
            ...
            for item in series {
                if String.IsNullOrEmpty(item.Id) || !ids.Add(item.Id!!) || String.IsNullOrWhiteSpace(item.Label) {
                    throw ArgumentException("Chart series require unique IDs and nonempty labels.")
                }
                if !Double.IsFinite(item.Value) || item.Value < 0.0 {
                    throw ArgumentOutOfRangeException("Chart series value")
                }
            ...
            for index in 0 ... series.Length {
                ...
                result[index] = ChartSegment{
                    Series: item,
                    Fraction: fraction,
                    Index: index,
                    Summary: item.Label + ": " + item.Value.ToString("G6", CultureInfo.InvariantCulture) +
                        " (" +
