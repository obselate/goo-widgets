...
public data struct StackedBar {
  ...
  public func Build() Blob {
    let resolved = this with{Width = Width ?? Length.Percent(100), Height = Height ?? 16.0, BorderRadius = BorderRadius ?? 4.0,
      TrackColor = TrackColor ?? Color.Parse("#27272a"), AccessibilityName = AccessibilityName ?? "Chart"}
    if !Double.IsFinite(resolved.Height!!) || resolved.Height!! <= 0.0 || !Double.IsFinite(resolved.BorderRadius!!) || resolved.BorderRadius!! < 0.0 { throw ArgumentOutOfRangeException("Bar height/radius") }
    let segments = ChartParts.Resolve(Series ?? []ChartSeries{})
    let plot = Container{Key: "plot", Width: Length.Percent(100), Height: resolved.Height!!, FlexShrink: 0, FlexDirection: FlexDirection.Row,
      BackgroundColor: resolved.TrackColor!!, BorderRadius: resolved.BorderRadius!!, Overflow: Overflow.Hidden}
    for segment in segments {
      if segment.Fraction <= 0.0 { continue }
      var bar = Container{BackgroundColor: segment.Series.Color!!, Focus: Style{Opacity: .7}, Cursor: OnActivate == nil ? Cursor.Default : Cursor.Pointer}
      if let create = CreateSegment { bar = create(resolved, segment, bar) }
