...
public data struct ListRow {
  ...
  public func Build() Blob {
    ...
    let resolved = this with{
      ...
      MinHeight = if MinHeight == 0.0 { 64.0 } else { MinHeight },
      PaddingHorizontal = if PaddingHorizontal == 0.0 { 14.0 } else { PaddingHorizontal },
      PaddingVertical = if PaddingVertical == 0.0 { 10.0 } else { PaddingVertical },
      Gap = if Gap == 0.0 { 12.0 } else { Gap }, ContentGap = if ContentGap == 0.0 { 3.0 } else { ContentGap },
      BorderRadius = BorderRadius ?? 8.0,
      TransitionMs = TransitionMs ?? 150.0,
      TransitionEasing = TransitionEasing ?? Easing.EaseOut,
      Opacity = Opacity ?? 1.0,
      TitleFontSize = if TitleFontSize == 0.0 { 14.0 } else { TitleFontSize },
      TitleFontWeight = if TitleFontWeight == 0 { 600 } else { TitleFontWeight },
      CaptionFontSize = if CaptionFontSize == 0.0 { 12.0 } else { CaptionFontSize },
      CaptionFontWeight = if CaptionFontWeight == 0 { 400 } else { CaptionFontWeight },
    ...
    let background = if resolved.Selected { resolved.SelectedBackgroundColor!! } else { resolved.BackgroundColor!! }
    let root = Container{
