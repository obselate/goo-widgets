...
public enum GridUnit { Auto; Fixed; Fraction }
...
public data struct GridTrack {
  ...
  shared {
    ...
    public func Auto(minimum float64 = 0.0, maximum float64? = nil) GridTrack -> GridTrack {Minimum: minimum, Maximum: maximum}
    ...
    public func Fraction(weight float64 = 1.0, minimum float64 = 0.0, maximum float64? = nil) GridTrack -> GridTrack {Unit: GridUnit.Fraction, Value: weight, Minimum: minimum, Maximum: maximum}
  }
}
...
internal class GridPolicy : LayoutAlgorithm {
  ...
  internal init(input Grid) {
    ...
    rowGap = input.RowGap
    if !Double.IsFinite(columnGap) || columnGap < 0.0 || !Double.IsFinite(rowGap) || rowGap < 0.0 { throw ArgumentOutOfRangeException("Grid gaps") }
    let keys = HashSet[string](StringComparer.Ordinal)
    for item in items {
      if String.IsNullOrEmpty(item.Id) || !keys.Add(item.Id!!) { throw ArgumentException("Grid items require unique, nonempty IDs.") }
      let rs = item.RowSpan ?? 1
      let cs = item.ColumnSpan ?? 1
      if rs < 1 || cs < 1 || item.Row < 0 || item.Column < 0 || rs > rows.Length || cs > columns.Length
        || item.Row > rows.Length - rs || item.Column > columns.Length - cs{ throw ArgumentOutOfRangeException("Grid item placement") }
    }
  ...
  private func Order(horizontal bool) []int32 {
    ...
    for index in 0 ... items.Length { order.Add(index) }
    order.Sort((a int32, b int32) -> {
  ...
  private func CopyTracks(source []GridTrack) []GridTrack {
    ...
    for track in result {
      ...
      if !Double.IsFinite(track.Minimum) || track.Minimum < 0.0 || Double.IsNaN(maximum) || maximum < track.Minimum
        || !Double.IsFinite(track.Value) || track.Value < 0.0 || (track.Unit == GridUnit.Fraction && track.Value == 0.0)
        || (track.Unit != GridUnit.Auto && track.Unit != GridUnit.Fixed && track.Unit != GridUnit.Fraction) { throw ArgumentOutOfRangeException("Grid track") }
      if track.Unit == GridUnit.Fraction { weightScale = Math.Max(weightScale, track.Value) }
    }
    if weightScale > 0.0 {
      for index in 0 ... result.Length {
        if result[index].Unit == GridUnit.Fraction { result[index] = result[index]with{Value = result[index].Value / weightScale} }
      }
    }
    return result
  }
  ...
  public func Measure(context LayoutContext, available LayoutSize) LayoutSize {
    ...
    return LayoutSize{Width: Math.Min(available.Width, Total(widths, columnGap)), Height: Math.Min(available.Height, Total(heights, rowGap))}
  }
  ...
  public func Arrange(context LayoutContext, finalSize LayoutSize) {
    ...
    for index in 0 ... items.Length {
      ...
      context.ArrangeChild(index, ElementRect{
        X: Offset(widths, item.Column, columnGap), Y: Offset(heights, item.Row, rowGap),
  ...
  private func MeasureColumns(context LayoutContext, available float64) []float64 {
    ...
    for index in columnOrder {
      let item = items[index]
      if Double.IsFinite(available) && HasFraction(columns, item.Column, item.ColumnSpan ?? 1) { continue }
      let measured = context.MeasureChild(index, LayoutSize{Width: Double.PositiveInfinity, Height: Double.PositiveInfinity})
      Grow(columns, sizes, item.Column, item.ColumnSpan ?? 1, measured.Width, columnGap, !Double.IsFinite(available))
    }
  ...
  private func MeasureRows(context LayoutContext, widths []float64, available float64) []float64 {
    ...
    for index in rowOrder {
      let item = items[index]
      if Double.IsFinite(available) && HasFraction(rows, item.Row, item.RowSpan ?? 1) { continue }
      let measured = context.MeasureChild(index, LayoutSize{Width: Span(widths, item.Column, item.ColumnSpan ?? 1, columnGap), Height: Double.PositiveInfinity})
  ...
  private func Initial(tracks []GridTrack) []float64 {
    ...
    for index in 0 ... tracks.Length {
      ...
      sizes[index] = track.Unit == GridUnit.Fixed ? Math.Clamp(track.Value, track.Minimum, track.Maximum ?? Double.PositiveInfinity) : track.Minimum
    }
    return sizes
  }

  private func HasFraction(tracks []GridTrack, start int32, count int32) bool {
    for index in start ... start + count { if tracks[index].Unit == GridUnit.Fraction { return true } }
    return false
  }

  private func Grow(tracks []GridTrack, sizes []float64, start int32, count int32, desired float64, gap float64, intrinsicFractions bool) {
    var remaining = desired - Span(sizes, start, count, gap)
    for pass in 0 ... count {
      if remaining <= .001 { break }
      var eligible int32
      for index in start ... start + count {
        if (tracks[index].Unit == GridUnit.Auto || (intrinsicFractions && tracks[index].Unit == GridUnit.Fraction))
          && sizes[index] < (tracks[index].Maximum ?? Double.PositiveInfinity) { eligible++ }
      }
      if eligible == 0 { break }
      let share = remaining / float64(eligible)
      var added = 0.0
      for index in start ... start + count {
        if tracks[index].Unit != GridUnit.Auto && !(intrinsicFractions && tracks[index].Unit == GridUnit.Fraction) { continue }
  ...
  private func Distribute(tracks []GridTrack, sizes []float64, available float64, gap float64) {
    ...
    for pass in 0 ... tracks.Length {
      ...
      var weight = 0.0
      for index in 0 ... tracks.Length {
        if tracks[index].Unit == GridUnit.Fraction && sizes[index] < (tracks[index].Maximum ?? Double.PositiveInfinity) { weight += tracks[index].Value }
      }
      if weight <= 0.0 { break }
      var added = 0.0
      for index in 0 ... tracks.Length {
        if tracks[index].Unit != GridUnit.Fraction { continue }
        let delta = Math.Min(remaining * tracks[index].Value / weight, (tracks[index].Maximum ?? Double.PositiveInfinity) - sizes[index])
        sizes[index] += delta
        added += delta
      }
      remaining -= added
