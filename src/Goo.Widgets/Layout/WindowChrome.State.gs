...
internal open class WindowChromeState : Cell[WindowChrome], IDisposable {
  ...
  protected override func Build(input WindowChrome) Blob {
    ...
    if !current.EnableContextMenu { menuOpen = false }
    let row = current.BuildResolved(BlankPointer)
    row.Key = "bar"
    let root = Container{Handle: rootHandle, FlexShrink: 0.0,
      OnPointerDown: PointerDown, OnKeyDown: KeyDown, Children: {row}}
    if current.EnableContextMenu {
      let maximized = current.IsMaximized!!
      let items = []MenuItem{
        MenuItem{Id: "restore", Label: "Restore", Disabled: !maximized || current.OnRestore == nil, OnActivate: current.OnRestore},
        MenuItem{Id: "minimize", Label: "Minimize", Disabled: current.OnMinimize == nil || host?.State == WindowState.Minimized, OnActivate: current.OnMinimize},
        MenuItem{Id: "maximize", Label: "Maximize", Disabled: maximized || current.OnMaximize == nil, OnActivate: current.OnMaximize},
      ...
      var prepared = MenuInput{Open: menuOpen, Items: items, OnDismiss: Dismiss,
        WindowPoint: menuPoint,
        Width: 200.0, MaxHeight: 240.0, AccessibilityName: "Window commands"}
      if menuOpen { prepared.Viewport = Viewport() }
      if menuPoint == nil { prepared.Anchor = rootHandle }
      var menu = if let create = current.CreateMenu { create(current, prepared) } else { prepared }
      menu.Open = menuOpen
      menu.Items = items
  ...
  private func Toggle() {
    ...
    if resolved.IsMaximized!! { resolved.OnRestore?.Invoke() }
    else { resolved.OnMaximize?.Invoke() }
  }
  private func NativeDoubleClick(event WindowTitlebarEvent) {
    if event.Handled || !current.EnableDoubleClick || !rootHandle.IsMounted || !Contains(rootBox, event.Position) { return }
    event.Handled = true
    Toggle()
  }
  private func PointerDown(event PointerEvent) {
    if !current.EnableContextMenu || menuOpen || event.Button != PointerButton.Secondary { return }
    event.PreventDefault()
    event.StopPropagation()
    Open(event.WindowPosition)
  }
  private func KeyDown(event KeyEvent) {
    if !current.EnableContextMenu || menuOpen { return }
    if event.Key != Key.Menu && !(event.Key == Key.F10 && event.Modifiers.Shift) { return }
    event.PreventDefault()
    event.StopPropagation()
    Open(nil)
  }
  ...
  private func Dismiss() { menuOpen = false
    menuPoint = nil
    Rebuild() }
  ...
  private func MenuRoot(input MenuInput, prepared Container) Container {
    let root = if let create = menuRootFactory { create(input, prepared) } else { prepared }
    let viewport = Viewport()
  ...
  private func StateChanged(state WindowState) { if !disposed { Rebuild() } }
  private func WindowMetricsChanged(metrics WindowMetrics) { if !disposed && menuOpen { Rebuild() } }
  private func BindOverlay(next ElementHandle?) {
    ...
    overlay = next
    if let handle = next { overlayBox = handle.BorderBox
      handle.MetricsChanged += OverlayMetrics }
  }
  private func OverlayMetrics(metrics ElementMetrics) {
    if disposed { return }
    if !metrics.IsMounted { if menuOpen { Dismiss() }
      return }
