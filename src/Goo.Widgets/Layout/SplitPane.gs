...
public enum SplitUnit { Ratio; Pixels }
...
public open class SplitPane : Cell[SplitPaneInput], IDisposable {
  ...
  protected override func Build(value SplitPaneInput) Blob {
    if !Double.IsFinite(value.Value) || !Double.IsFinite(value.MinimumFirst) || !Double.IsFinite(value.MinimumSecond)
      || value.MinimumFirst < 0.0 || value.MinimumSecond < 0.0 { throw ArgumentOutOfRangeException("Value/minimum sizes") }
    let step = value.Step ?? (value.Unit == SplitUnit.Ratio ? .02 : 8.0)
    let size = value.HandleSize ?? 6.0
    if !Double.IsFinite(step) || step <= 0.0 || !Double.IsFinite(size) || size < 0.0 { throw ArgumentOutOfRangeException("Step/HandleSize") }
    input = value with{Step = step, HandleSize = size, Width = value.Width ?? Length.Percent(100), Height = value.Height ?? Length.Percent(100), AccessibilityName = value.AccessibilityName ?? "Resize panes"}
    if input.Disabled { pointer = -1L }
    ...
    let second = Container{Key: "second", MinWidth: 0, MinHeight: 0, FlexGrow: 1, FlexBasis: 0, Overflow: Overflow.Hidden}
    if horizontal { first.Width = pixels } else { first.Height = pixels }
    if let content = input.First { first.Children.Add(content) }
    if let content = input.Second { second.Children.Add(content) }
    var divider = Container{BackgroundColor: "#3f3f46", Hover: Style{BackgroundColor: "#71717a"}, Focus: Style{BackgroundColor: "#a1a1aa"}}
    if let create = input.CreateHandle { divider = create(input, divider) }
    divider.Key = "divider"
    ...
    if horizontal { divider.Width = size } else { divider.Height = size }
    ...
    divider.Accessibility = Accessibility{
      Role: AccessibilityRole.Slider, Name: input.AccessibilityName!!,
      Orientation: horizontal ? AccessibilityOrientation.Horizontal : AccessibilityOrientation.Vertical,
      Range: AccessibilityValue{Minimum: 0, Maximum: input.Unit == SplitUnit.Ratio ? 1.0 : available, Now: lastRequest, Text: lastRequest.ToString("0.##", CultureInfo.InvariantCulture)},
      Actions: []AccessibilityAction{AccessibilityAction.Increment, AccessibilityAction.Decrement, AccessibilityAction.SetValue},
  ...
  private func Metrics(metrics ElementMetrics) {
    ...
    let next = Math.Max(0, (input.Orientation == SplitOrientation.Horizontal ? metrics.ContentBox.Width : metrics.ContentBox.Height) - (input.HandleSize ?? 6))
    if Math.Abs(next - available) > .01 { available = next
      Rebuild() }
  }
  private func Clamp(value float64) float64 {
    ...
    return Math.Clamp(value, input.MinimumFirst * factor, Math.Max(input.MinimumFirst * factor, available - input.MinimumSecond * factor))
  }
  private func ToValue(pixels float64) float64 -> input.Unit == SplitUnit.Ratio ? (available > 0.0 ? pixels / available : 0.0) : pixels
  private func Coordinate(e PointerEvent) float64 -> input.Orientation == SplitOrientation.Horizontal ? e.WindowPosition.X : e.WindowPosition.Y
  ...
  private func Begin(e PointerEvent) {
    if input.Disabled || pointer != -1L || e.Button != PointerButton.Primary { return }
    pointer = e.PointerId
    origin = Coordinate(e)
  ...
  private func Move(e PointerEvent) { if !input.Disabled && pointer == e.PointerId { Request(originSize + Coordinate(e) - origin) } }
  ...
  private func Cancel(e PointerEvent) { if pointer == e.PointerId { e.ReleaseCapture()
    pointer = -1L } }
  private func KeyDown(e KeyEvent) {
    ...
    if !decrease && !increase && e.Key != Key.Home && e.Key != Key.End { return }
    e.PreventDefault()
  ...
  private func AccessibilityAction(request AccessibilityActionRequest) bool {
    ...
    if request.Action == Goo.AccessibilityAction.Increment { value += input.Step!! }
    else if request.Action == Goo.AccessibilityAction.Decrement { value -= input.Step!! }
    else if request.Action == Goo.AccessibilityAction.SetValue {
      if !Double.TryParse(request.Value, NumberStyles.Float, CultureInfo.InvariantCulture, out value) || !Double.IsFinite(value) { return false }
    } else { return false }
