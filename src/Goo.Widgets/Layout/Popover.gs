...
public enum PopoverPlacement { BottomStart; BottomEnd; TopStart; TopEnd; RightStart; LeftStart }
/// The interaction or anchor lifecycle event requesting dismissal.
public enum PopoverDismissReason { Escape; OutsideClick; AnchorRemoved }
...
public open class Popover : Cell[PopoverInput], IDisposable {
  ...
  protected override func Build(input PopoverInput) Blob {
    ...
    if !input.Open {
      ...
      panelHeight = 0.0
      return Container{HitTestSelf: false, Accessibility: Accessibility{Role: AccessibilityRole.None, Hidden: true}}
    }
    ...
    if let handle = anchor {
      if handle.IsMounted { anchorBox = handle.BorderBox
    ...
    let panel = if let create = current.CreatePanel { create(current, bounds, content) }
    else { Container{
      Padding: current.Padding!!, BackgroundColor: current.BackgroundColor!!,
      BorderColor: current.BorderColor!!, BorderWidth: 1.0, BorderRadius: 8.0,
    } }
    panel.Key = "panel"
    ...
    let backdrop = Container{
      Key: "backdrop", Position: PositionType.Absolute,
      Left: 0.0, Right: 0.0, Top: 0.0, Bottom: 0.0,
      BackgroundColor: Color.Transparent, Focusable: false, Cursor: Cursor.Default,
    ...
    let root = if let create = current.CreateRoot { create(current, backdrop, panel) }
    else { Container{
    ...
    for overlay in current.Overlays ?? []Blob {} { root.Children.Add(overlay) }
    return root
  }
  ...
  private func Resolve(input PopoverInput) PopoverInput {
    let result = input with{
      ...
      Gap = input.Gap ?? 4.0, ViewportMargin = input.ViewportMargin ?? 8.0,
      ...
      BorderColor = input.BorderColor ?? Color.Parse("#3f3f46"), Padding = input.Padding ?? 8.0,
    }
    ...
    if let point = result.WindowPoint {
      if !Double.IsFinite(point.X) || !Double.IsFinite(point.Y) { throw ArgumentOutOfRangeException("WindowPoint") }
    }
    if !Enum.IsDefined(result.Placement) { throw ArgumentOutOfRangeException("Placement") }
    return result
  }
  ...
  private func Place(viewport ElementRect, width float64, height float64) Point {
    ...
    if let point = current.WindowPoint {
      return Point{X: Math.Clamp(point.X, left, Math.Max(left, right - width)),
    ...
    if current.Placement == PopoverPlacement.TopStart || current.Placement == PopoverPlacement.TopEnd {
      ...
      if y < top && anchorBox.Y + anchorBox.Height + gap + height <= bottom { y = anchorBox.Y + anchorBox.Height + gap }
      ...
      if current.Placement == PopoverPlacement.RightStart {
        ...
        if x + width > right && anchorBox.X - gap - width >= left { x = anchorBox.X - gap - width }
      } else {
        x = anchorBox.X - width - gap
        if x < left && anchorBox.X + anchorBox.Width + gap + width <= right { x = anchorBox.X + anchorBox.Width + gap }
      }
    } else if y + height > bottom && anchorBox.Y - height - gap >= top { y = anchorBox.Y - height - gap }
    return Point{X: Math.Clamp(x, left, Math.Max(left, right - width)),
      Y: Math.Clamp(y, top, Math.Max(top, bottom - height))}
  }

  private func BindAnchor(next ElementHandle?) {
    if anchor == next { return }
    if let previous = anchor { previous.MetricsChanged -= AnchorMetrics }
    anchor = next
    anchorWasMounted = false
    anchorDismissed = false
    if let handle = anchor { handle.MetricsChanged += AnchorMetrics }
  }
  ...
  private func AnchorMetrics(metrics ElementMetrics) {
    ...
    anchorWasMounted = true
    if metrics.BorderBox != anchorBox { anchorBox = metrics.BorderBox
      Rebuild() }
  }
  ...
  private func RootMetrics(metrics ElementMetrics) {
    ...
    if !metrics.IsMounted { CloseScope()
      return }
    if metrics.BorderBox != rootBox { rootBox = metrics.BorderBox
      Rebuild() }
  ...
  private func PanelMetrics(metrics ElementMetrics) {
    if disposed || !current.Open || !metrics.IsMounted { return }
    if Math.Abs(metrics.BorderBox.Height - panelHeight) > 0.01 { panelHeight = metrics.BorderBox.Height
      Rebuild() }
  }
  ...
  private func HandleKey(event KeyEvent) {
    if event.Key == Key.Escape {
      ...
      if !event.Repeat { Dismiss(PopoverDismissReason.Escape) }
      return
    }
    rootKeyHandler?.Invoke(event)
  }

  private func Dismiss(reason PopoverDismissReason) { current.OnDismiss?.Invoke(reason) }
