...
public data struct Disclosure {
  ...
  public func Build() Blob {
    ...
    let resolved = this with{
      ...
      CreateIndicator = nil, CreateHeader = nil, CreateContent = nil, CreateRoot = nil,
    }
    let indicator = if let create = createIndicator { create(resolved) } else {
      MaterialIcons.Create(if resolved.Expanded { "expand_more" } else { "chevron_right" },
        size: 20.0, color: resolved.TextColor!!)
    }
    ...
    var header = Button{
      ...
      Disabled: resolved.Disabled, Focusable: !resolved.Disabled,
      Cursor: if resolved.Disabled { Cursor.Default } else { Cursor.Pointer },
      OnClick: action,
      Accessibility: Accessibility{
        Role: AccessibilityRole.Button, Name: resolved.AccessibilityName!!, Expanded: resolved.Expanded,
      },
      Children: {indicator, heading},
    }
    if let create = createHeader { header = create(resolved, header) }
    var body = Container{
      Key: "content", Padding: resolved.ContentPadding!!, MinWidth: 0,
      Display: if resolved.Expanded { Display.Flex } else { Display.None },
      Accessibility: Accessibility{Role: AccessibilityRole.Group, Hidden: !resolved.Expanded},
    }
    if let content = resolved.Content { body.Children.Add(content) }
    if let create = createContent { body = create(resolved, body) }
    if let create = createRoot { return create(resolved, header, body) }
    return Container{
      Width: resolved.Width!!, MinWidth: 0, FlexShrink: 0,
      BackgroundColor: resolved.BackgroundColor!!,
      BorderColor: resolved.BorderColor!!, BorderWidth: resolved.BorderWidth!!,
      BorderRadius: resolved.BorderRadius!!, Overflow: Overflow.Hidden,
