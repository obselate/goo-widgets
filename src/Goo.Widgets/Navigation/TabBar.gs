...
public open class TabBar : Cell[TabBarInput] {
  ...
  protected override func Build(value TabBarInput) Blob {
    ...
    input = value with{Items = items, AccessibilityName = value.AccessibilityName ?? "Tabs", Width = value.Width ?? Length.Percent(100), ItemHeight = value.ItemHeight ?? 40.0}
    if !Double.IsFinite(input.ItemHeight!!) || input.ItemHeight!! < 0.0 { throw ArgumentOutOfRangeException("ItemHeight") }
    let ids = HashSet[string](StringComparer.Ordinal)
    var first string?
    for item in items {
      if String.IsNullOrEmpty(item.Id) || !ids.Add(item.Id!!) { throw ArgumentException("Tab IDs must be nonempty and unique") }
      if !handles.ContainsKey(item.Id!!) { handles.Add(item.Id!!, ElementHandle()) }
      if first == nil && !item.Disabled { first = item.Id }
    }
    let removed = List[string]()
    for entry in handles { if !ids.Contains(entry.Key) { removed.Add(entry.Key) } }
    ...
    for source in items {
      let id = source.Id!!
      let item = source with{Label = source.Label ?? id, AccessibilityName = source.AccessibilityName ?? source.Label ?? id}
      let active = id == input.SelectedId
      var button = Button{
        Opacity: item.Disabled ? .45 : 1.0, Cursor: item.Disabled ? Cursor.Default : Cursor.Pointer,
        MinHeight: input.ItemHeight!!, PaddingLeft: 16, PaddingRight: 16, FlexShrink: 0,
        Color: active ? "#fafafa" : "#a1a1aa", BackgroundColor: active ? "#3f3f46" : "#18181b",
        Hover: Style{BackgroundColor: "#27272a"}, Focus: Style{OutlineWidth: 1, OutlineColor: "#a1a1aa", OutlineOffset: -2},
        Children: {item.Content ?? Text{Content: item.Label!!, TextWrap: TextWrap.NoWrap}},
      }
      if let create = input.CreateItem { button = create(input, item, button) }
      button.Key = id
      ...
      button.OnFocus = (e FocusEvent) -> { focusedId = id
        hasFocus = true
        Rebuild() }
      button.OnBlur = (e FocusEvent) -> { hasFocus = false
        Rebuild() }
      button.OnKeyDown = (e KeyEvent) -> KeyDown(id, e)
      button.Accessibility = Accessibility{Role: AccessibilityRole.Tab, Name: item.AccessibilityName!!, Selected: active}
      buttons.Add(button)
    }
    ...
    root.Accessibility = Accessibility{Role: AccessibilityRole.TabList, Name: input.AccessibilityName!!, Orientation: AccessibilityOrientation.Horizontal}
    root.Children.Clear()
    for button in buttons { root.Children.Add(button) }
    return root
  }

  private func Enabled(id string?) bool {
    if id == nil { return false }
    for item in items { if item.Id == id { return !item.Disabled } }
    return false
  }
  private func Select(id string) { if Enabled(id) { input.OnSelect?.Invoke(id) } }
  private func KeyDown(id string, e KeyEvent) {
    if !Enabled(id) { return }
    var index = -1
    for i in 0 ... items.Length { if items[i].Id == id { index = i
      break } }
    var direction int32
    if e.Key == Key.Right { direction = 1 }
    else if e.Key == Key.Left { direction = -1 }
    else if e.Key == Key.Home { direction = 1
      index = -1 }
    else if e.Key == Key.End { direction = -1
      index = items.Length }
    else { return }
    e.PreventDefault()
    e.StopPropagation()
    for attempt in 0 ... items.Length {
      ...
      if input.Activation == TabActivation.Automatic { Select(item.Id!!) }
      return
    }
  }
}
