...
public open class Menu : Cell[MenuInput] {
  ...
  protected override func Build(input MenuInput) Blob {
    ...
    return Level(input.Items ?? []MenuItem{}, 0, input.Anchor, input.WindowPoint,
      input.AccessibilityName ?? "Menu")
  }

  private func Validate(source []MenuItem) {
    items.Clear()
    let pending = Stack[MenuItem]()
    for item in source { pending.Push(item) }
    while pending.Count > 0 {
      ...
      if item.Separator && children.Length > 0 { throw ArgumentException("Menu separators cannot own submenus") }
      for child in children { pending.Push(child) }
      if !handles.ContainsKey(id) { handles.Add(id, ElementHandle()) }
    }
    let removed = List[string]()
    for id in handles.Keys { if !items.ContainsKey(id) { removed.Add(id) } }
  ...
  private func Level(source []MenuItem, depth int32, anchor ElementHandle?, point Point?, name string) Blob {
    if depth > 64 { throw ArgumentException("Menus support at most 64 submenu levels") }
    var selected = active.TryGetValue(depth, out var prior) ? prior : ""
    if !Eligible(source, selected) { selected = First(source, false) }
    active[depth] = selected
    let rows = Container{Gap: 2.0, Accessibility: Accessibility{Role: AccessibilityRole.None}}
    for item in source {
      let id = item.Id!!
      if item.Separator {
        let separator = if let create = current.CreateSeparator { create(current, item) }
        else { Container{Height: 1.0, MarginTop: 4.0, MarginBottom: 4.0, BackgroundColor: "#3f3f46"} }
        separator.Key = id
      ...
      let content = item.Content ?? Text{Content: label, FontSize: 14, Color: item.Disabled ? Color.Parse("#71717a") : Color.Parse("#e4e4e7")}
      let prepared = Button{
        MinHeight: 32.0, PaddingLeft: 10.0, PaddingRight: 8.0, PaddingTop: 6.0, PaddingBottom: 6.0,
        FlexDirection: FlexDirection.Row, AlignItems: AlignItems.Center, Gap: 14.0,
        BackgroundColor: selected == id ? Color.Parse("#34334c") : Color.Transparent,
        BorderRadius: 4.0, Cursor: item.Disabled ? Cursor.Default : Cursor.Pointer,
      }
      let row = if let create = current.CreateItem { create(current, item, prepared) } else { prepared }
      row.Key = id
      ...
      row.Accessibility = Accessibility{
        Role: AccessibilityRole.MenuItem, Name: label,
    ...
    if let anchor = anchor { popup.Anchor = anchor }
    if let point = point { popup.WindowPoint = point }
    if let viewport = current.Viewport { popup.Viewport = viewport }
    if selected != "" { popup.InitialFocus = handles[selected] }
  ...
  private func Panel(input PopoverInput, bounds ElementRect, content Blob) Container {
    let prepared = Container{Padding: 4.0, BackgroundColor: "#18181b", BorderColor: "#3f3f46", BorderWidth: 1.0, BorderRadius: 7.0}
    return if let create = current.CreatePanel { create(current, prepared) } else { prepared }
  }
  ...
  private func Root(input PopoverInput, backdrop Container, panel Container) Container {
    ...
    return if let create = current.CreateRoot { create(current, prepared) } else { prepared }
  }

  private func Eligible(source []MenuItem, id string) bool {
    for item in source { if item.Id == id { return !item.Disabled && !item.Separator } }
    return false
  }
  ...
  private func First(source []MenuItem, last bool) string {
    for step in 0 ... source.Length {
      ...
      if !item.Disabled && !item.Separator { return item.Id!! }
    }
    return ""
  }
  ...
  private func Move(source []MenuItem, depth int32, id string, direction int32) {
    var index = 0
    for i in 0 ... source.Length { if source[i].Id == id { index = i } }
    for step in 1 ... source.Length + 1 {
      let next = (index + direction * step + source.Length) % source.Length
      if !source[next].Disabled && !source[next].Separator { Focus(depth, source[next].Id!!)
        return }
    }
  }

  private func Focus(depth int32, id string) {
    if id == "" || !handles.TryGetValue(id, out var handle) { return }
    active[depth] = id
    handle.Focus()
  ...
  private func Truncate(depth int32) {
    if path.Count > depth { path.RemoveRange(depth, path.Count - depth) }
  }

  private func OpenChild(depth int32, id string) {
    let children = items[id].Children ?? []MenuItem{}
    if children.Length == 0 { return }
    if path.Count > depth && path[depth] == id { return }
    Truncate(depth)
    path.Add(id)
  ...
  private func Hover(depth int32, id string) {
    if !items.TryGetValue(id, out var item) || item.Disabled || item.Separator { return }
    Focus(depth, id)
    if (item.Children ?? []MenuItem {}).Length > 0 { OpenChild(depth, id) }
    else { Truncate(depth) }
    Rebuild()
  }
  ...
  private func Activate(depth int32, id string) {
    ...
    if (item.Children ?? []MenuItem {}).Length > 0 { OpenChild(depth, id)
      return }
    if let callback = item.OnActivate { callback() } else { current.OnActivate?.Invoke(id) }
    if current.DismissOnActivate ?? true { current.OnDismiss?.Invoke() }
  }

  private func CloseLevel(depth int32) {
    if depth == 0 { current.OnDismiss?.Invoke()
      return }
