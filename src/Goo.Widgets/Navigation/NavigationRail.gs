...
public data struct NavigationRail {
  ...
  public func Build() Blob {
    ...
    let resolved = this with{
      ...
      CollapsedWidth = if CollapsedWidth == 0.0 { 64.0 } else { CollapsedWidth },
      ExpandedWidth = if ExpandedWidth == 0.0 { 220.0 } else { ExpandedWidth },
      Height = Height ?? Length.Percent(100.0),
      ItemHeight = if ItemHeight == 0.0 { 44.0 } else { ItemHeight },
      Gap = Gap ?? 4.0,
      ...
      FontSize = if FontSize == 0.0 { 13.0 } else { FontSize },
    ...
    for index in 0 ... items.Length {
      ...
      let item = source with{
        Id = id,
        Label = label,
        AccessibilityName = source.AccessibilityName ?? label,
      }
      if let createItem = createItem {
        ...
        let button = Button{
          ...
          BackgroundColor: if selected { resolved.SelectedBackgroundColor!! } else { resolved.ItemBackgroundColor!! },
          Color: if selected { resolved.SelectedTextColor!! } else { resolved.TextColor!! },
          Hover: Style{BackgroundColor: resolved.HoverBackgroundColor!!},
          Cursor: if item.Disabled { Cursor.Default } else { Cursor.Pointer },
          Focusable: true,
          Disabled: item.Disabled,
          OnClick: onClick,
          FlexDirection: FlexDirection.Row,
          AlignItems: AlignItems.Center,
          JustifyContent: if resolved.Expanded { JustifyContent.FlexStart } else { JustifyContent.Center },
    ...
    if let createRoot = createRoot { return createRoot(resolved, buttons) }
    let root = Container{
      Width: if resolved.Expanded { resolved.ExpandedWidth } else { resolved.CollapsedWidth },
