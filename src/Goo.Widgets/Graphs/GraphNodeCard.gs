...
public data struct GraphNodeCard {
  ...
  public func Build() Container {
    let width = if Width == 0.0 { 160.0 } else { Width }
    let height = if Height == 0.0 { 64.0 } else { Height }
    ...
    return Container{
      ...
      BorderColor: if Selected { SelectedBorderColor ?? Color.Parse("#93c5fd") } else { BorderColor ?? Color.Parse("#52525b") },
      BackgroundColor: if Selected { SelectedBackgroundColor ?? Color.Parse("#1d4ed8") } else { BackgroundColor ?? Color.Parse("#18181b") },
