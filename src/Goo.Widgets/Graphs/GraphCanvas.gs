...
enum GraphPointerMode { None; Pan; Marquee; Node }
...
public open class GraphCanvas : Cell[GraphCanvasInput] {
  ...
  private func Resolve(input GraphCanvasInput) GraphCanvasInput {
    let minimumZoom = if input.MinimumZoom == 0.0 { 0.1 } else { input.MinimumZoom }
    let maximumZoom = if input.MaximumZoom == 0.0 { 4.0 } else { input.MaximumZoom }
    if !Double.IsFinite(minimumZoom) || minimumZoom <= 0.0 { throw ArgumentOutOfRangeException("MinimumZoom") }
    if !Double.IsFinite(maximumZoom) || maximumZoom < minimumZoom { throw ArgumentOutOfRangeException("MaximumZoom") }
    let value = input with{
      ...
      Viewport = GraphViewport{
        ...
        Zoom: Math.Clamp(if input.Viewport.Zoom == 0.0 { 1.0 } else { input.Viewport.Zoom }, minimumZoom, maximumZoom),
      },
      Width = if input.Width == 0.0 { 800.0 } else { input.Width },
      Height = if input.Height == 0.0 { 480.0 } else { input.Height },
      NodeWidth = if input.NodeWidth == 0.0 { 160.0 } else { input.NodeWidth },
      NodeHeight = if input.NodeHeight == 0.0 { 64.0 } else { input.NodeHeight },
      GridSpacing = if input.GridSpacing == 0.0 { 48.0 } else { input.GridSpacing },
      MinimumGridScreenSpacing = if input.MinimumGridScreenSpacing == 0.0 { 18.0 } else { input.MinimumGridScreenSpacing },
      MinimumZoom = minimumZoom,
      MaximumZoom = maximumZoom,
      EdgeWidth = if input.EdgeWidth == 0.0 { 2.0 } else { input.EdgeWidth },
  ...
  private func Validate(value GraphCanvasInput) {
    if !Double.IsFinite(value.Width) || value.Width <= 0.0 { throw ArgumentOutOfRangeException("Width") }
    if !Double.IsFinite(value.Height) || value.Height <= 0.0 { throw ArgumentOutOfRangeException("Height") }
    if !Double.IsFinite(value.NodeWidth) || value.NodeWidth <= 0.0 { throw ArgumentOutOfRangeException("NodeWidth") }
    if !Double.IsFinite(value.NodeHeight) || value.NodeHeight <= 0.0 { throw ArgumentOutOfRangeException("NodeHeight") }
    if !Double.IsFinite(value.GridSpacing) || value.GridSpacing <= 0.0 { throw ArgumentOutOfRangeException("GridSpacing") }
    ...
    if !Double.IsFinite(value.EdgeWidth) || value.EdgeWidth <= 0.0 { throw ArgumentOutOfRangeException("EdgeWidth") }
    if !Double.IsFinite(value.Viewport.PanX) || !Double.IsFinite(value.Viewport.PanY)
      || !Double.IsFinite(value.Viewport.Zoom) { throw ArgumentOutOfRangeException("Viewport") }
  ...
  private func GridLayer() Container {
    ...
    while spacing < resolved.MinimumGridScreenSpacing { spacing *= 2.0 }
    var offsetX = viewport.PanX % spacing
    var offsetY = viewport.PanY % spacing
    if offsetX < 0.0 { offsetX += spacing }
    if offsetY < 0.0 { offsetY += spacing }
    var index int32
    var x = offsetX
    while x <= resolved.Width {
      layer.Children.Add(Container{
        Key: "grid-x-" + index.ToString(), Position: PositionType.Absolute,
        Left: x, Top: 0.0, Width: 1.0, Height: resolved.Height,
        BackgroundColor: resolved.GridColor!!, HitTestSelf: false,
      })
      x += spacing
      index++
    }
    ...
    while y <= resolved.Height {
      layer.Children.Add(Container{
        Key: "grid-y-" + index.ToString(), Position: PositionType.Absolute,
        Left: 0.0, Top: y, Width: resolved.Width, Height: 1.0,
        BackgroundColor: resolved.GridColor!!, HitTestSelf: false,
      })
      y += spacing
      index++
  ...
  private func EdgeLayer() Container {
    ...
    for node in resolved.Nodes { nodes[node.Id] = node }
    for edge in resolved.Edges {
      ...
      if Math.Max(fromPosition.X, toPosition.X) < 0.0 || Math.Min(fromPosition.X, toPosition.X) > resolved.Width
        || Math.Max(fromPosition.Y, toPosition.Y) < 0.0 || Math.Min(fromPosition.Y, toPosition.Y) > resolved.Height{ continue }
      ...
      let stroke = Container{
        ...
        BackgroundColor: if selected { resolved.SelectedEdgeColor!! } else { resolved.EdgeColor!! },
        Opacity: if selected { 1.0 } else if resolved.SelectedNodeIds.Length > 0 { 0.25 } else { 0.7 },
  ...
  private func PassiveLayer(key string) Container -> Container {
    Key: key, Position: PositionType.Absolute, Left: 0.0, Top: 0.0,
    Width: resolved.Width, Height: resolved.Height, HitTestSelf: false,
  }
  ...
  private func Marquee() Container {
    ...
    return Container{
      Key: "graph-marquee", Position: PositionType.Absolute,
      Left: left, Top: top,
  ...
  private func BeginCanvasPointer(e PointerEvent) {
    if pointerMode != GraphPointerMode.None { return }
    if e.Button == PointerButton.Middle {
      ...
      if !marqueeAdd && !marqueeToggle { EmitSelection([]string{}) }
    } else {
      return
    }
  ...
  private func BeginNodePointer(node GraphNode, e PointerEvent) {
    if e.Button != PointerButton.Primary || pointerMode != GraphPointerMode.None { return }
  ...
  private func EmitSelection(ids []string) { resolved.OnSelectionChanged?.Invoke(ids) }

  private func Zoom(e WheelEvent) {
    if e.Delta.Y == 0.0 { return }
