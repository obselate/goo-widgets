...
public data struct DismissibleContextBar {
  ...
  public func Build() Blob {
    ...
    let resolved = this with{
      AccessibilityName = AccessibilityName ?? "Context", DismissText = dismissText,
      DismissAccessibilityName = DismissAccessibilityName ?? dismissText, Width = Width ?? Length.Percent(100.0),
      BackgroundColor = BackgroundColor ?? Color.Parse("#18181b"), TextColor = TextColor ?? Color.Parse("#fafafa"),
      BorderColor = BorderColor ?? Color.Parse("#3f3f46"), DismissBorderColor = DismissBorderColor ?? Color.Transparent,
      DismissBackgroundColor = DismissBackgroundColor ?? Color.Transparent, DismissTextColor = DismissTextColor ?? Color.Parse("#a1a1aa"),
      DismissHoverBackgroundColor = DismissHoverBackgroundColor ?? Color.Parse("#27272a"), DismissActiveBackgroundColor = DismissActiveBackgroundColor ?? Color.Parse("#3f3f46"),
      DismissFocusBorderColor = DismissFocusBorderColor ?? Color.Parse("#d4d4d8"), DismissDisabledBackgroundColor = DismissDisabledBackgroundColor ?? Color.Transparent,
      DismissDisabledTextColor = DismissDisabledTextColor ?? Color.Parse("#71717a"), MinHeight = if MinHeight == 0.0 { 44.0 } else { MinHeight },
      PaddingHorizontal = if PaddingHorizontal == 0.0 { 12.0 } else { PaddingHorizontal }, PaddingVertical = if PaddingVertical == 0.0 { 8.0 } else { PaddingVertical },
      Gap = Gap ?? 10.0, BorderWidth = BorderWidth ?? 1.0, DismissBorderWidth = DismissBorderWidth ?? 1.0,
      BorderRadius = BorderRadius ?? 8.0, DismissBorderRadius = DismissBorderRadius ?? 5.0,
      TransitionMs = TransitionMs ?? 150.0, TransitionEasing = TransitionEasing ?? Easing.EaseOut,
      DismissActiveTransform = DismissActiveTransform ?? PanelTransform{Scale: 0.98}, Opacity = Opacity ?? 1.0,
      DismissHeight = if DismissHeight == 0.0 { 28.0 } else { DismissHeight }, DismissMinWidth = if DismissMinWidth == 0.0 { 72.0 } else { DismissMinWidth },
      DismissPaddingHorizontal = if DismissPaddingHorizontal == 0.0 { 10.0 } else { DismissPaddingHorizontal },
      DismissFontSize = if DismissFontSize == 0.0 { 12.0 } else { DismissFontSize }, DismissFontWeight = if DismissFontWeight == 0 { 600 } else { DismissFontWeight },
      Transform = Transform ?? PanelTransform{}, CreateDismiss = nil, CreateRoot = nil,
    }
    ...
    if let createDismiss = createDismiss {
      ...
      button = Button{
        Height: resolved.DismissHeight, MinWidth: resolved.DismissMinWidth,
        PaddingLeft: resolved.DismissPaddingHorizontal, PaddingRight: resolved.DismissPaddingHorizontal,
        BorderWidth: resolved.DismissBorderWidth!!, BorderColor: resolved.DismissBorderColor!!,
        BorderRadius: resolved.DismissBorderRadius!!, BackgroundColor: resolved.DismissBackgroundColor!!, Color: resolved.DismissTextColor!!,
        FlexShrink: 0.0, Cursor: Cursor.Pointer, Focusable: true, Disabled: resolved.Disabled, OnClick: resolved.OnDismiss, TransitionMs: resolved.TransitionMs!!, TransitionEasing: resolved.TransitionEasing!!,
        Hover: Style{BackgroundColor: resolved.DismissHoverBackgroundColor!!},
        Active: Style{BackgroundColor: resolved.DismissActiveBackgroundColor!!, Transform: resolved.DismissActiveTransform!!},
        Focus: if resolved.ShowFocusHighlight { Style{BorderColor: resolved.DismissFocusBorderColor!!} } else { Style{} },
        DisabledStyle: Style{BackgroundColor: resolved.DismissDisabledBackgroundColor!!, Color: resolved.DismissDisabledTextColor!!},
        Accessibility: Accessibility{Role: AccessibilityRole.Button, Name: resolved.DismissAccessibilityName!!},
        Children: { label },
      }
    }
    if let createRoot = createRoot { return createRoot(resolved, Content, button!!) }
    let root = Container{
      FlexDirection: FlexDirection.Row, Width: resolved.Width!!, MinHeight: resolved.MinHeight,
      PaddingLeft: resolved.PaddingHorizontal, PaddingRight: resolved.PaddingHorizontal,
      PaddingTop: resolved.PaddingVertical, PaddingBottom: resolved.PaddingVertical,
      Gap: resolved.Gap!!, BackgroundColor: resolved.BackgroundColor!!, Color: resolved.TextColor!!, BorderColor: resolved.BorderColor!!,
      BorderWidth: resolved.BorderWidth!!, BorderRadius: resolved.BorderRadius!!,
      Opacity: resolved.Opacity!!, Transform: resolved.Transform!!, TransitionMs: resolved.TransitionMs!!, TransitionEasing: resolved.TransitionEasing!!,
