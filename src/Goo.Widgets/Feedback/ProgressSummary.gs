...
public data struct ProgressSummary {
  ...
  public func Build() Blob {
    ...
    let resolved = this with{
      ...
      LabelFontWeight = if LabelFontWeight == 0 { 600 } else { LabelFontWeight },
      DetailFontSize = if DetailFontSize == 0.0 { 12.0 } else { DetailFontSize },
      CreateProgress = nil,
      CreateRoot = nil,
    }
    ...
    if resolved.FontFamily != nil { labelText.FontFamily = resolved.FontFamily!! }
    ...
    if Detail != nil {
      let detailText = Text{
        Content: Detail!!,
        Color: resolved.DetailColor!!,
        FontSize: resolved.DetailFontSize,
      }
    ...
    if let createRoot = createRoot { return createRoot(resolved, header, progress) }
    return Container{
