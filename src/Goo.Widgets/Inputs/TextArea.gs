...
public open class TextArea : Cell[TextAreaInput], IDisposable {
  ...
  protected override func Build(value TextAreaInput) Blob {
    if disposed { throw ObjectDisposedException("TextArea") }
    ...
    if rows < 1 || !Double.IsFinite(font) || font <= 0.0 || !Double.IsFinite(minimum) || minimum < 0.0 { throw ArgumentOutOfRangeException("MinimumRows/FontSize/MinimumHeight") }
    let defaultWidth Length = 280.0
    let defaultHeight Length = minimum
    input = value with{MinimumRows = rows, MinimumHeight = minimum, FontSize = font, Width = value.Width ?? defaultWidth, Placeholder = value.Placeholder ?? "", AccessibilityName = value.AccessibilityName ?? value.Label ?? value.Placeholder ?? "", BackgroundColor = value.BackgroundColor ?? Color.Parse("#09090b"), TextColor = value.TextColor ?? Color.Parse("#fafafa"), InvalidColor = value.InvalidColor ?? Color.Parse("#ef4444"), FocusColor = value.FocusColor ?? Color.Parse("#d4d4d8")}
    if controller == nil {
      external = value.Controller
      ownsController = external == nil
      controller = external ?? TextEditorController(TextDocument(value.Value ?? ""))
    } else if value.Controller != external { throw ArgumentException("Controller cannot change within one mounted TextArea identity") }
    let editorController = controller!!
    if let next = value.Value { Synchronize(next) }
    var editor = TextEditor(editorController, value.Layers ?? []TextPresentationLayer{}) {
      BasedOn = value.EditorStyle, Width = Length.Percent(100), Height = value.Height ?? defaultHeight, MinHeight = minimum,
      Padding = 12, FontSize = font, BackgroundColor = input.BackgroundColor!!, Color = input.TextColor!!,
      BorderWidth = 1, BorderColor = value.Invalid ? input.InvalidColor!! : Color.Parse("#3f3f46"), BorderRadius = 6,
    ...
    if let family = input.FontFamily { editor.FontFamily = family }
    if let create = input.CreateEditor { editor = create(input, editor) }
    if editor.Controller != editorController { throw ArgumentException("CreateEditor must preserve the TextArea controller") }
    editor.Key = "editor"
    ...
    editor.Accessibility = Accessibility{Role: AccessibilityRole.TextEditor, Name: input.AccessibilityName!!, Invalid: input.Invalid,
    ...
    var root = Container{BasedOn: input.RootStyle, Width: input.Width!!, MinWidth: 0, Gap: 6, FlexDirection: FlexDirection.Column}
    if let create = input.CreateRoot { root = create(input, root) }
    root.FlexDirection = FlexDirection.Column
    if let family = input.FontFamily { root.FontFamily = family }
    root.Children.Clear()
    if let label = input.Label { root.Children.Add(Text{Key: "label", Handle: labelHandle, Content: label, Color: "#a1a1aa", FontSize: 11, FontWeight: 600}) }
    root.Children.Add(editor)
    if let issue = input.IssueText { root.Children.Add(Text{Key: "issue", Handle: issueHandle, Content: issue, Color: input.InvalidColor!!, FontSize: 12,
      Accessibility: Accessibility{Role: AccessibilityRole.Alert, Name: issue, Live: AccessibilityLive.Polite}}) }
    return root
  }

  private func Changed(change TextDocumentChange) {
    if !syncing && !disposed { input.OnChange?.Invoke(controller!!.Document.GetText()) }
  }
  private func Synchronize(next string) {
    ...
    var suffix int32
    while suffix < before.Length - prefix && suffix < next.Length - prefix && before[before.Length - suffix - 1] == next[next.Length - suffix - 1] { suffix++ }
    if suffix > 0 && Char.IsLowSurrogate(before[before.Length - suffix]) { suffix-- }
    syncing = true
    try {
      owned.Document.Apply(TextChange{Range: TextRange{Start: prefix, Length: before.Length - prefix - suffix}, InsertedText: next.Substring(prefix, next.Length - prefix - suffix)})
      let anchor = Math.Min(selection.Anchor.Offset, next.Length)
      let active = Math.Min(selection.Active.Offset, next.Length)
      try {
        owned.Selection = TextSelection{Anchor: TextPosition{Offset: anchor, Affinity: selection.Anchor.Affinity}, Active: TextPosition{Offset: active, Affinity: selection.Active.Affinity}}
