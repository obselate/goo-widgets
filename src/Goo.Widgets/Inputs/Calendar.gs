...
public open class Calendar : Cell[CalendarInput], IDisposable {
  ...
  public func Dispose() {
    if disposed { return }
    disposed = true
    for handle in handles.Values { handle.MetricsChanged -= DayMetrics }
    handles.Clear()
  }
  ...
  protected override func Build(input CalendarInput) Blob {
    ...
    current = input with{Culture = culture, FirstDayOfWeek = input.FirstDayOfWeek ?? culture.DateTimeFormat.FirstDayOfWeek,
      Width = input.Width ?? width, DayHeight = input.DayHeight == 0.0 ? 32.0 : input.DayHeight,
      Today = input.Today ?? DateOnly.FromDateTime(DateTime.Today), AccessibilityName = input.AccessibilityName ?? "Calendar",
      PreviousMonthLabel = input.PreviousMonthLabel ?? "Previous month", NextMonthLabel = input.NextMonthLabel ?? "Next month"}
    if !Double.IsFinite(current.DayHeight) || current.DayHeight <= 0.0 { throw ArgumentOutOfRangeException("DayHeight") }
    if !Enum.IsDefined(current.FirstDayOfWeek!!) { throw ArgumentOutOfRangeException("FirstDayOfWeek") }
    if input.Minimum != nil && input.Maximum != nil && input.Minimum!! > input.Maximum!! { throw ArgumentException("Calendar minimum must not exceed maximum") }
    if !(culture.DateTimeFormat.Calendar is GregorianCalendar) { throw ArgumentException("Calendar requires a culture using the Gregorian calendar") }
    if initialized && firstWeekday != current.FirstDayOfWeek!! {
      if focusedDay != nil { pending = focusedDay }
      for handle in handles.Values { handle.MetricsChanged -= DayMetrics }
      handles.Clear()
    }
    ...
    if input.Minimum != nil && month < Start(input.Minimum!!) { month = Start(input.Minimum!!) }
    if input.Maximum != nil && month > Start(input.Maximum!!) { month = Start(input.Maximum!!) }
    if input.Month == nil { ownedMonth = month }
    ...
    let heading = Container{Key: "heading", Height: 32.0, FlexShrink: 0.0, FlexDirection: FlexDirection.Row, AlignItems: AlignItems.Center, Gap: 8.0,
      Children: {Navigation(-1), Text{Key: "month", Content: title, FlexGrow: 1.0, TextAlign: TextAlign.Center, FontWeight: 600, Color: "#fafafa",
        Accessibility: Accessibility{Role: AccessibilityRole.Heading, Name: title, Level: 2, Live: AccessibilityLive.Polite}}, Navigation(1)}}
    let grid = Container{Key: "grid", Gap: 2.0, Accessibility: Accessibility{Role: AccessibilityRole.Grid, Name: title}}
    let weekdays = Container{Key: "weekdays", Height: 24.0, FlexDirection: FlexDirection.Row, Accessibility: Accessibility{Role: AccessibilityRole.Row}}
    for column in 0 ... 7 {
      let weekday = DayOfWeek((int32(current.FirstDayOfWeek!!) + column) % 7)
      let prepared = Text{Content: culture.DateTimeFormat.GetShortestDayName(weekday), Color: "#a1a1aa", FontSize: 12.0, TextAlign: TextAlign.Center}
      let label = if let create = current.CreateWeekday { create(current, weekday, prepared) } else { prepared }
      label.Key = "weekday-" + column.ToString()
      label.FlexGrow = 1.0
      label.FlexBasis = 0.0
      label.MinWidth = 0.0
      label.Accessibility = Accessibility{Role: AccessibilityRole.ColumnHeader, Name: culture.DateTimeFormat.GetDayName(weekday)}
    ...
    for week in 0 ... 6 {
      let row = Container{Key: "week-" + (first + week * 7).ToString(), FlexDirection: FlexDirection.Row, Gap: 2.0, Accessibility: Accessibility{Role: AccessibilityRole.Row}}
      for column in 0 ... 7 {
        let number = first + week * 7 + column
        if number < DateOnly.MinValue.DayNumber || number > DateOnly.MaxValue.DayNumber {
          row.Children.Add(Container{Key: "empty-" + column.ToString(), FlexGrow: 1.0, FlexBasis: 0.0, Height: current.DayHeight, Accessibility: Accessibility{Hidden: true}})
          continue
        }
        let date = DateOnly.FromDayNumber(number)
        visible.Add(number)
        row.Children.Add(Day(CalendarDay{Date: date, CurrentMonth: Start(date) == month, Selected: input.Value == date,
          Today: current.Today == date, Active: active == date, Disabled: !Allowed(date)}))
      }
      grid.Children.Add(row)
    }
    let removed = List[int32]()
    for pair in handles { if !visible.Contains(pair.Key) { pair.Value.MetricsChanged -= DayMetrics
      removed.Add(pair.Key) } }
    for number in removed { handles.Remove(number) }
    var root = Container{Width: current.Width!!, MinWidth: 0.0, Padding: 8.0, Gap: 8.0,
      BackgroundColor: "#18181b", BorderRadius: 7.0, Color: "#fafafa"}
    ...
    root.OnFocus = (event FocusEvent) -> { event.StopPropagation()
      if active != nil { Focus(active!!) } }
  ...
  private func InMonth(value DateOnly, preferred int32) DateOnly? {
    ...
    for distance in 0 ... count {
      if day + distance <= count { let date = DateOnly(value.Year, value.Month, day + distance)
        if Allowed(date) { return date } }
      if distance > 0 && day - distance > 0 { let date = DateOnly(value.Year, value.Month, day - distance)
        if Allowed(date) { return date } }
    }
    return nil
  }
  private func CanMonth(delta int32) bool {
    ...
    if current.Disabled || number < 0 || number >= 9999 * 12 { return false }
    let target = month.AddMonths(delta)
    return (current.Minimum == nil || target >= Start(current.Minimum!!)) && (current.Maximum == nil || target <= Start(current.Maximum!!))
  }
  private func Navigation(delta int32) Button {
    ...
    if let create = current.CreateNavigation { button = create(current, delta, button) }
    ...
    button.Accessibility = Accessibility{Role: AccessibilityRole.Button, Name: delta < 0 ? current.PreviousMonthLabel!! : current.NextMonthLabel!!}
  ...
  private func Day(day CalendarDay) Button {
    let date = day.Date
    var button = Button{Padding: 0.0, BorderRadius: 4.0,
      BackgroundColor: day.Selected ? Color.Parse("#4f46e5") : Color.Transparent,
      BorderWidth: day.Today ? 1.0 : 0.0, BorderColor: "#818cf8",
      Focus: Style{OutlineWidth: 1.0, OutlineColor: "#c7d2fe", OutlineOffset: -2.0},
      Opacity: day.Disabled ? .35 : 1.0}
    if let create = current.CreateDay { button = create(current, day, button) }
    ...
    button.OnFocus = (event FocusEvent) -> { event.StopPropagation()
      ...
      focusedDay = date
      Rebuild() }
    button.OnBlur = (event FocusEvent) -> { if focusedDay == date { focusedDay = nil } }
    button.Accessibility = Accessibility{Role: AccessibilityRole.GridCell, Name: date.ToString("D", current.Culture!!), Selected: day.Selected,
      Description: day.Today ? "Today" : ""}
    button.Children.Clear()
    button.Children.Add(Text{Content: date.Day.ToString(current.Culture!!), FontSize: 13.0,
  ...
  private func Focus(date DateOnly) {
    ...
    if pending == nil { handle.ScrollIntoView() }
    Rebuild()
  }
  private func DayMetrics(metrics ElementMetrics) {
    if disposed || pending == nil || !metrics.IsMounted { return }
    let target = pending!!
  ...
  private func KeyDown(event KeyEvent) {
    ...
    else if event.Key == Key.Right { step = 1 }
    else if event.Key == Key.Up { step = -7 }
    else if event.Key == Key.Down { step = 7 }
    else if event.Key == Key.Home || event.Key == Key.End {
      let offset = (int32(origin.DayOfWeek) - int32(current.FirstDayOfWeek!!) + 7) % 7
      number -= offset
      if event.Key == Key.End { number += 6 }
    ...
    for attempt in 0 ... attempts {
      number += step
      if number < DateOnly.MinValue.DayNumber || number > DateOnly.MaxValue.DayNumber { return }
      let date = DateOnly.FromDayNumber(number)
      if Allowed(date) { if Start(date) != month { ShowMonth(date) }
        Focus(date)
        return }
    }
  }
