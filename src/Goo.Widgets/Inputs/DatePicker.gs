...
public open class DatePicker : Cell[DatePickerInput], IDisposable {
  ...
  protected override func Build(input DatePickerInput) Blob {
    current = input with{AllowEmpty = input.AllowEmpty ?? true, Culture = input.Culture ?? CultureInfo.CurrentCulture,
      Width = input.Width == 0.0 ? 280.0 : input.Width, PopupWidth = input.PopupWidth == 0.0 ? 300.0 : input.PopupWidth,
      ZIndex = input.ZIndex == 0 ? 20 : input.ZIndex, AccessibilityName = input.AccessibilityName ?? "Date",
      Placeholder = input.Placeholder ?? "Choose a date"}
    if !Double.IsFinite(current.Width) || current.Width <= 40.0 || !Double.IsFinite(current.PopupWidth) || current.PopupWidth <= 40.0 {
      throw ArgumentOutOfRangeException("DatePicker dimensions")
    }
    if input.Minimum != nil && input.Maximum != nil && input.Minimum!! > input.Maximum!! { throw ArgumentException("DatePicker minimum must not exceed maximum") }
    if !(current.Culture!!.DateTimeFormat.Calendar is GregorianCalendar) { throw ArgumentException("DatePicker requires a culture using the Gregorian calendar") }
    if !initialized || input.Value != lastValue {
      draft = Display(input.Value)
      dirty = false
      invalid = input.Value != nil && !Allowed(input.Value!!)
      initialized = true
    } else if !dirty { draft = Display(input.Value)
      invalid = input.Value != nil && !Allowed(input.Value!!) }
    lastValue = input.Value
    BindOverlay(input.OverlayHost)
    let requestedOpen = !input.Disabled && (input.Open ?? ownedOpen)
    if requestedOpen && overlay == nil { throw InvalidOperationException("An open DatePicker requires an OverlayHost") }
    let isOpen = requestedOpen && (overlay?.IsMounted ?? false)
    var entry = TextEntry{Height: 38.0, MinWidth: 0.0, PaddingLeft: 10.0, PaddingRight: 6.0,
      FlexGrow: 1.0, FlexBasis: 0.0, BackgroundColor: Color.Transparent, Color: "#fafafa"}
    if let create = current.CreateInput { entry = create(current, entry) }
    entry.Key = "entry"
    ...
    entry.OnBlur = (event FocusEvent) -> { if !(current.Open ?? ownedOpen) { Submit() } }
    entry.OnKeyDown = InputKey
    entry.Accessibility = Accessibility{Role: AccessibilityRole.TextInput, Name: current.AccessibilityName!!,
      Invalid: invalid, Required: !current.AllowEmpty!!,
      Relationships: AccessibilityRelationships{Controls: []ElementHandle{calendarHandle}, ErrorMessage: invalid ? []ElementHandle{issueHandle} : []ElementHandle{}}}
    var button = Button{Width: 36.0, Height: 38.0, Padding: 8.0, BackgroundColor: Color.Transparent}
    if let create = current.CreateButton { button = create(current, button) }
    button.Key = "open"
    button.Disabled = current.Disabled
    button.Focusable = !current.Disabled
    button.OnClick = () -> SetOpen(!isOpen)
    button.Accessibility = Accessibility{Role: AccessibilityRole.Button, Name: current.AccessibilityName!! +" calendar", HasPopup: true, Expanded: isOpen,
    ...
    let field = Container{Key: "field", Handle: fieldHandle, FlexDirection: FlexDirection.Row, AlignItems: AlignItems.Center,
      BackgroundColor: "#18181b", BorderColor: invalid ? Color.Parse("#f87171") : Color.Parse("#52525b"), BorderWidth: 1.0, BorderRadius: 6.0,
    ...
    if invalid { root.Children.Add(Text{Key: "issue", Handle: issueHandle, Content: current.InvalidMessage ?? "Enter a valid date.", Color: "#f87171", FontSize: 12.0,
      Accessibility: Accessibility{Role: AccessibilityRole.Alert, Name: current.InvalidMessage ?? "Enter a valid date.", Live: AccessibilityLive.Polite}}) }
    var calendar = CalendarInput{}
    if let create = current.CreateCalendar { calendar = create(current, calendar) }
    calendar = calendar with{Value = current.Value, OnChange = Selected, Minimum = current.Minimum, Maximum = current.Maximum,
      IsDateDisabled = current.IsDateDisabled, Disabled = current.Disabled, Culture = current.Culture,
      Today = current.Today, FirstDayOfWeek = current.FirstDayOfWeek, Handle = calendarHandle, Width = Length.Percent(100),
      OnEscape = Escape, AccessibilityName = current.AccessibilityName!! +" calendar"}
    var popup = PopoverInput{Open: isOpen, Anchor: fieldHandle, Width: current.PopupWidth, MaxHeight: 360.0,
      Content: Cell.Mount[CalendarInput, Calendar]("calendar", calendar), Padding: 0.0,
      AccessibilityName: current.AccessibilityName!! +" popup", InitialFocus: calendarHandle,
      OnDismiss: Dismiss, CreateRoot: PopupRoot, CreatePanel: PopupPanel}
  ...
  private func Display(value DateOnly?) string {
    if value == nil { return "" }
    return if let format = current.Format { format(value!!) } else { value!!.ToString("d", current.Culture!!) }
  }
  ...
  private func Edited(value string) { draft = value
    dirty = true
    invalid = false
    Rebuild() }
  private func Submit() {
    if current.Disabled || !dirty { return }
    var parsed DateOnly?
    if String.IsNullOrWhiteSpace(draft) {
      if !current.AllowEmpty!! { Reject()
        return }
    } else {
      if let parse = current.Parse { parsed = parse(draft) }
      else if DateOnly.TryParse(draft, current.Culture!!, DateTimeStyles.None, out var date) { parsed = date }
      if parsed == nil || !Allowed(parsed!!) { Reject()
        return }
    }
    Apply(parsed)
  }
  private func Reject() { invalid = true
  ...
  private func Apply(value DateOnly?) {
    ...
    dirty = false
    invalid = false
    if value != current.Value { current.OnChange?.Invoke(value) }
    Rebuild()
  }
  private func Selected(date DateOnly) { if !Allowed(date) { return }
    Apply(date)
    SetOpen(false) }
  private func SetOpen(value bool) { if current.Disabled { return }
    ownedOpen = value
    current.OnOpenChange?.Invoke(value)
    Rebuild() }
  private func Escape() { draft = Display(current.Value)
    dirty = false
    invalid = false
    SetOpen(false) }
  private func Dismiss(reason PopoverDismissReason) {
    if reason == PopoverDismissReason.Escape { Escape() } else { SetOpen(false) }
  }
  ...
  private func PopupPanel(input PopoverInput, bounds ElementRect, content Blob) Container {
    let prepared = Container{BackgroundColor: "#18181b", BorderColor: "#52525b", BorderWidth: 1.0, BorderRadius: 7.0}
  ...
  private func OverlayMetrics(metrics ElementMetrics) {
    if disposed { return }
    if !metrics.IsMounted { if current.Open ?? ownedOpen { SetOpen(false) }
      return }
