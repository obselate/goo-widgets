...
public data struct Stepper {
  ...
  public func Build() Blob {
    ...
    let resolved = this with{
      Labels = rawLabels, CurrentIndex = current,
      Width = Width ?? Length.Percent(100.0), Height = if Height == 0.0 { 32.0 } else { Height },
      Gap = Gap ?? 8.0, PaddingHorizontal = PaddingHorizontal ?? 12.0, BorderRadius = BorderRadius ?? 16.0,
      CompletedColor = CompletedColor ?? Color.Parse("#d4d4d8"), CurrentColor = CurrentColor ?? Color.Parse("#fafafa"),
      UpcomingColor = UpcomingColor ?? Color.Parse("#27272a"), ActiveTextColor = ActiveTextColor ?? Color.Parse("#09090b"),
      UpcomingTextColor = UpcomingTextColor ?? Color.Parse("#a1a1aa"),
      FontSize = if FontSize == 0.0 { 12.0 } else { FontSize }, FontWeight = if FontWeight == 0 { 600 } else { FontWeight },
      TransitionMs = TransitionMs ?? 150.0, TransitionEasing = TransitionEasing ?? Easing.EaseOut,
      Opacity = Opacity ?? 1.0, Transform = Transform ?? PanelTransform{},
      CreateLabel = nil, CreateStep = nil, CreateRoot = nil,
    }
    ...
    for index in 0 ... count {
      ...
      if let createLabel = createLabel {
        ...
        let textColor = if index <= current { resolved.ActiveTextColor!! } else { resolved.UpcomingTextColor!! }
        text = Text{
          Content: labelText, Color: textColor, FontSize: resolved.FontSize,
          FontWeight: resolved.FontWeight, Accessibility: Accessibility{Hidden: true},
        }
      }
      ...
      if let createStep = createStep {
        ...
        pill = Container{
          Width: Length.Percent(100.0), Height: resolved.Height,
          PaddingLeft: resolved.PaddingHorizontal!!, PaddingRight: resolved.PaddingHorizontal!!,
          BorderRadius: resolved.BorderRadius!!, BackgroundColor: fillColor,
          TransitionMs: resolved.TransitionMs!!, TransitionEasing: resolved.TransitionEasing!!, FlexGrow: 1.0,
          AlignItems: AlignItems.Center, JustifyContent: JustifyContent.Center, Children: { text!! },
        }
      }
      pills[index] = pill!!
    }
    ...
    if count > 0 {
      let accessibilityRange = AccessibilityValue{
        Minimum: 0.0, Maximum: float64(count - 1),
        Now: float64(current), Text: resolved.AccessibilityValueText ?? rawLabels[current],
      }
      if resolved.AccessibilityName != nil {
        semantics = Accessibility{Role: AccessibilityRole.ProgressBar, Name: resolved.AccessibilityName!!, Range: accessibilityRange}
      } else {
    ...
    let root = Container{
      Width: resolved.Width!!, Gap: resolved.Gap!!,
      Opacity: resolved.Opacity!!, Transform: resolved.Transform!!,
      FlexDirection: FlexDirection.Row, Accessibility: semantics,
    }
    for i in 0 ... count {
      root.Children.Add(Container{Key: "${i}", FlexGrow: 1.0, Children: { pills[i] }})
    }
    return root
  }
}
