...
public open class MediaTransport : Cell[MediaTransportInput] {
    protected override func Build(input MediaTransportInput) Blob {
        ...
        let previous = ActionButton(
            ...
            resolved.OnPrevious,
            !resolved.CanPrevious!!
        )
        let playbackAction = if resolved.Playing {
                ...
                Container(){
                    ...
                    Cell.Mount[SliderInput, Slider](
                        ...
                        SliderInput{
                            ...
                            Minimum: 0.0,
                            Maximum: if resolved.Duration > 0.0 {
                                resolved.Duration
                            } else {
                                1.0
                            },
        ...
        if createRoot != nil {
            return createRoot(resolved, root)
        }
        return root
    }

    private func Resolve(input MediaTransportInput) MediaTransportInput {
        if !Double.IsFinite(input.Position) {
            throw ArgumentOutOfRangeException("Position")
        }
        if !Double.IsFinite(input.Duration) || input.Duration < 0.0 {
            throw ArgumentOutOfRangeException("Duration")
        }
        if !Double.IsFinite(input.Volume) {
            throw ArgumentOutOfRangeException("Volume")
        }
        let actionSize = if input.ActionSize == 0.0 {
            40.0
        } else {
            input.ActionSize
        }
        let primarySize = if input.PrimaryActionSize == 0.0 {
            52.0
        } else {
            input.PrimaryActionSize
        }
        if !Double.IsFinite(actionSize) || actionSize <= 0.0 {
            throw ArgumentOutOfRangeException("ActionSize")
        }
        if !Double.IsFinite(primarySize) || primarySize <= 0.0 {
            throw ArgumentOutOfRangeException("PrimaryActionSize")
        }
        ...
        return input with{
            ...
            CanPlayPause = !input.Disabled &&
                (input.CanPlayPause ?? input.OnPlayPause != nil) &&
                input.OnPlayPause != nil,
    ...
    shared {
        ...
        public func FormatTime(value float64) string {
            let finite = if Double.IsFinite(value) {
                value
            } else {
                0.0
            }
