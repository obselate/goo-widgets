...
public data struct MediaCard {
  ...
  public func Build() Blob {
    ...
    let title = Title ?? ""
    let width = if Width == 0.0 { 200.0 } else { Width }
    let resolved = this with{
      Title = title,
      AccessibilityName = AccessibilityName ?? title,
      Width = width,
      ThumbnailHeight = if ThumbnailHeight == 0.0 { width } else { ThumbnailHeight },
      ContentHeight = if ContentHeight == 0.0 { 88.0 } else { ContentHeight },
      ...
      SupportingFontSize = if SupportingFontSize == 0.0 { 13.0 } else { SupportingFontSize },
      CreateContent = nil,
      CreateRoot = nil,
    }
