...
public data struct Chip {
  ...
  public func Build() Blob {
    ...
    let resolved = this with{
      ...
      Height = if Height == 0.0 { 28.0 } else { Height }, PaddingHorizontal = if PaddingHorizontal == 0.0 { 10.0 } else { PaddingHorizontal },
      Gap = if Gap == 0.0 { 6.0 } else { Gap }, FontSize = if FontSize == 0.0 { 12.0 } else { FontSize }, FontWeight = if FontWeight == 0 { 600 } else { FontWeight },
      BorderWidth = BorderWidth ?? 1.0, BorderRadius = BorderRadius ?? 14.0,
      TransitionMs = TransitionMs ?? 150.0, TransitionEasing = TransitionEasing ?? Easing.EaseOut,
      Opacity = Opacity ?? 1.0, DisabledOpacity = DisabledOpacity ?? 0.5,
      Transform = Transform ?? PanelTransform{}, ActiveTransform = ActiveTransform ?? PanelTransform{Scale: 0.98},
    ...
    let currentText = if resolved.Selected { resolved.SelectedTextColor!! } else { resolved.TextColor!! }
    let currentBorder = if resolved.Selected { resolved.SelectedBorderColor!! } else { resolved.BorderColor!! }
    let currentHoverBg = if resolved.Selected { resolved.SelectedHoverBackgroundColor!! } else { resolved.HoverBackgroundColor!! }
    let currentActiveBg = if resolved.Selected { resolved.SelectedActiveBackgroundColor!! } else { resolved.ActiveBackgroundColor!! }

    let root = Button{
      Height: resolved.Height, PaddingLeft: resolved.PaddingHorizontal, PaddingRight: resolved.PaddingHorizontal,
      Gap: resolved.Gap, BorderWidth: resolved.BorderWidth!!, BorderRadius: resolved.BorderRadius!!,
      BorderColor: currentBorder, BackgroundColor: currentBg, Color: currentText,
      Opacity: resolved.Opacity!!, Transform: resolved.Transform!!, Cursor: Cursor.Pointer, Focusable: true,
      Disabled: resolved.Disabled, OnClick: resolved.OnClick,
      ...
      Focus: if resolved.ShowFocusHighlight { Style{BorderColor: resolved.FocusBorderColor!!} } else { Style{} }, DisabledStyle: Style{Opacity: resolved.DisabledOpacity!!},
      FlexDirection: FlexDirection.Row, AlignItems: AlignItems.Center,
      Accessibility: Accessibility{
        Role: AccessibilityRole.Checkbox, Name: resolved.AccessibilityName!!,
        Checked: if resolved.Selected { AccessibilityChecked.True } else { AccessibilityChecked.False },
      },
    }
