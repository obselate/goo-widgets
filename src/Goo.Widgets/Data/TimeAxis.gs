...
public enum TimeAxisAlignment { Center; Start; Nearest }
...
public open class TimeAxis : Cell[TimeAxisInput], IDisposable {
  ...
  public init() { viewport.MetricsChanged += Metrics }
  ...
  protected override func Build(value TimeAxisInput) Blob {
    input = value with{TickInterval = value.TickInterval ?? TimeSpan.FromHours(1), PixelsPerHour = value.PixelsPerHour ?? 120.0,
      MaximumLanes = value.MaximumLanes ?? 4, LaneHeight = value.LaneHeight ?? 30.0, LaneGap = value.LaneGap ?? 6.0,
      MinimumEventWidth = value.MinimumEventWidth ?? 6.0, MarkerLabel = value.MarkerLabel ?? "Now", MarkerColor = value.MarkerColor ?? Color.Parse("#fb7185"),
      Width = value.Width ?? Length.Percent(100), AccessibilityName = value.AccessibilityName ?? "Timeline"}
    let duration = input.End - input.Start
    if duration.Ticks <= 0 || input.TickInterval!!.Ticks <= 0 { throw ArgumentOutOfRangeException("TimeAxis range/tick interval") }
    if duration.Ticks / input.TickInterval!!.Ticks >= 4096L { throw ArgumentOutOfRangeException("TimeAxis has more than 4096 ticks") }
    ...
    if !Double.IsFinite(scale) || scale <= 0.0 || !Double.IsFinite(canvasWidth) || canvasWidth > 1e9 || canvasWidth < .01
      || !Double.IsFinite(height) || height <= 0.0 || !Double.IsFinite(gap) || gap < 0.0 || !Double.IsFinite(minimum) || minimum <= 0.0
      || input.MaximumLanes!! < 1 || input.MaximumLanes!! > 1024 { throw ArgumentOutOfRangeException("TimeAxis scale/lanes") }
    events = input.Events ?? []TimeAxisEvent{}
    let ids = HashSet[string](StringComparer.Ordinal)
    let visible = List[TimeAxisEvent]()
    for item in events {
      if String.IsNullOrEmpty(item.Id) || !ids.Add(item.Id!!) || String.IsNullOrWhiteSpace(item.Label) { throw ArgumentException("TimeAxis events require unique IDs and labels.") }
      if item.End < item.Start { throw ArgumentOutOfRangeException("TimeAxis event interval") }
      if item.Start < input.End && (item.End > input.Start || (item.Start == item.End && item.Start == input.Start)) { visible.Add(item) }
    }
    visible.Sort((a TimeAxisEvent, b TimeAxisEvent) -> a.Start == b.Start ? StringComparer.Ordinal.Compare(a.Id, b.Id) : a.Start.CompareTo(b.Start))
    ...
    for item in visible {
      ...
      var lane int32
      while lane < ends.Count && ends[lane] > left { lane++ }
      if lane >= input.MaximumLanes!! { overflow.Add(item)
        continue }
      if lane == ends.Count { ends.Add(right) } else { ends[lane] = right }
    ...
    let canvas = Container{Width: canvasWidth, Height: canvasHeight, FlexShrink: 0, Position: PositionType.Relative, Overflow: Overflow.Hidden}
    ...
    for index in 0 ... tickCount {
      ...
      let tick = TimeAxisTick{Time: time, X: X(time), Label: input.FormatTick == nil ? time.ToString("HH:mm", CultureInfo.CurrentCulture) : input.FormatTick!! (time)}
      var blob = Container{Width: 1, Height: canvasHeight, BackgroundColor: "#34343e", HitTestSelf: false,
        Children: {Text{Content: tick.Label!!, Position: PositionType.Absolute, Left: 5, Top: 3, FontSize: 11, Color: "#a1a1aa", TextWrap: TextWrap.NoWrap}}}
      if let create = input.CreateTick { blob = create(input, tick, blob) }
    ...
    for placement in placements {
      ...
      var blob = Button{PaddingLeft: 8, PaddingRight: 8, BackgroundColor: selected ? Color.Parse("#4f46a5") : Color.Parse("#313b50"), BorderRadius: 4,
        BorderWidth: selected ? 2.0 : 0.0, BorderColor: "#a5b4fc", Color: "#e4e4e7", Hover: Style{Opacity: .85}, Focus: Style{BorderWidth: 2, BorderColor: "#e0e7ff"},
        Overflow: Overflow.Hidden, JustifyContent: JustifyContent.Center, Children: {Text{Content: item.Label!!, FontSize: 12, TextWrap: TextWrap.NoWrap}}}
      if placement.Width < 40.0 { blob.Children.Clear() }
      if let create = input.CreateEvent { blob = create(input, placement, blob) }
      ...
      blob.Accessibility = Accessibility{Role: AccessibilityRole.Button, Name: EventName(item), Selected: selected}
      canvas.Children.Add(blob)
    }
    if overflow.Count > 0 {
      ...
      for item in hidden { right = Math.Min(canvasWidth, Math.Max(right, X(item.End))) }
      var blob = Button{PaddingLeft: 6, PaddingRight: 6, BackgroundColor: "#292936", Color: "#c4b5fd", BorderRadius: 4, Overflow: Overflow.Hidden,
        Children: {Text{Content: "+" + hidden.Length.ToString() + " more", FontSize: 12, TextWrap: TextWrap.NoWrap}}}
      if let create = input.CreateOverflow { blob = create(input, hidden, blob) }
      blob.Key = "overflow"
      ...
      for item in hidden { names.Add(EventName(item)) }
      blob.Accessibility = Accessibility{Role: AccessibilityRole.Button, Name: hidden.Length.ToString() + " overlapping events", Description: String.Join("; ", names)}
      canvas.Children.Add(blob)
    }
    if let time = input.CurrentTime {
      if time >= input.Start && time < input.End {
        var marker = Container{Width: 2, Height: canvasHeight, BackgroundColor: input.MarkerColor!!, HitTestSelf: false,
          Children: {Text{Content: input.MarkerLabel!!, Position: PositionType.Absolute, Left: 5, Top: 16, FontSize: 10, Color: input.MarkerColor!!, TextWrap: TextWrap.NoWrap}}}
        if let create = input.CreateMarker { marker = create(input, marker) }
        marker.Key = "marker"
    ...
    var root = Container{Handle: viewport, Focusable: true, Width: input.Width!!, Height: input.Height ?? naturalHeight, MinWidth: 0, BackgroundColor: "#17171e", BorderRadius: 6}
  ...
  private func EventName(item TimeAxisEvent) string -> item.Label + ", " + item.Start.ToString("g", CultureInfo.CurrentCulture) + " – " + item.End.ToString("g", CultureInfo.CurrentCulture)
  private func At(x float64) DateTimeOffset {
    let ticks = (input.End - input.Start).Ticks
    return input.Start.AddTicks(Math.Clamp(int64(Math.Clamp(x / canvasWidth, 0.0, 1.0) * float64(ticks)), 0L, ticks))
  }
  private func ApplyInitial(width float64, scrollRange float64) bool {
    if width <= 0.0 || initialDone { return false }
    let eventId = input.InitialEventId
    var target = input.InitialTime
    var targetEnd = target
    if !String.IsNullOrEmpty(eventId) {
      target = nil
      for item in events { if item.Id == eventId { target = item.Start
        targetEnd = item.End
        break } }
    }
    if let time = target {
      ...
      var offset = left
      if alignment == TimeAxisAlignment.Center { offset = (left + right - width) / 2.0 }
    ...
    } else { lastOffset = viewport.ScrollOffset.X }
    lastWidth = width
    initialDone = true
    return true
  }
  private func Metrics(metrics ElementMetrics) {
    ...
    if ApplyInitial(metrics.ContentBox.Width, metrics.ScrollRange.X) { return }
    if Math.Abs(lastOffset - metrics.ScrollOffset.X) < .01 && Math.Abs(lastWidth - metrics.ContentBox.Width) < .01 { return }
    lastOffset = metrics.ScrollOffset.X
    lastWidth = metrics.ContentBox.Width
    input.OnViewportChanged?.Invoke(TimeAxisViewport{Start: At(lastOffset), End: At(lastOffset + lastWidth), Offset: lastOffset})
  }
  private func KeyDown(e KeyEvent) {
    let current = viewport.ScrollOffset.X
    var next = current
    if e.Key == Key.Left { next -= input.PixelsPerHour!! / 4.0 }
    else if e.Key == Key.Right { next += input.PixelsPerHour!! / 4.0 }
    else if e.Key == Key.Home { next = 0 }
    else if e.Key == Key.End { next = viewport.ScrollRange.X }
    else { return }
    e.PreventDefault()
