...
5. Create an annotated matching tag, for example `git tag -a v0.1.1 -m "Goo Widgets 0.1.1"`, and push the tag.
6. The tag run verifies and packs the library, publishes it through NuGet Trusted Publishing, and creates a GitHub release with the package and its checksum. Portable debug symbols are embedded in the library.
